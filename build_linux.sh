...
# Check CMake version. Upstream OrcaSlicer 2.4.x builds its dependencies with CMake 4.x
# (CMAKE_POLICY_VERSION_MINIMUM is forwarded to every dependency and the GitHub-like
# container runner (-g) installs CMake 4.3), so CMake 4.x is no longer rejected here;
# only report it, in case an older dependency recipe still trips over CMake 4 behaviour.
...
if [ "$CMAKE_MAJOR" -ge 4 ] 2>/dev/null; then
    echo ""
    echo "================================================================"
    echo " NOTE: CMake ${CMAKE_VER} (4.x) detected."
    echo " CMAKE_POLICY_VERSION_MINIMUM=3.5 is exported for dependency builds."
    echo " If a dependency fails during CMake configure, retry with"
    echo " CMake 3.28.x – 3.31.x (https://github.com/Kitware/CMake/releases)."
    echo "================================================================"
    echo ""
fi
