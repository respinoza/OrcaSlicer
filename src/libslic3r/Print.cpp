...
StringObjectException Print::validate(StringObjectException *warning, Polygons* collison_polygons, std::vector<std::pair<Polygon, float>>* height_polygons) const
{
    ...
    // Snapmaker: upstream validates mixed nozzle temperatures here via check_multi_filament_valid()
    // (nozzles < 2 && extruders.size() > 1, error or warning depending on need_check_multi_filaments_compatibility()).
    // The fork (Snapmaker "Feature top cover" #504) deliberately keeps this call disabled: its GUI owns the
    // high/low-temperature mixing check (Plater check_filament_temp_mixing + confirm-before-slice dialog using
    // filament_is_high_temperature). Re-enabling upstream's check would block slices the fork lets the user confirm.
    ...
    if (warning) {
        try {
            ...
            else if (m_default_object_config.default_junction_deviation.values.front() > max_junction_deviation) {
