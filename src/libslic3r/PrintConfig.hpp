...
enum PrintHostType {
    // Snapmaker: htMoonRaker_mqtt / htMoonRaker keep their fork positions (their int values are stored as SSWCP protocol ids);
    // upstream's ht3DPrinterOS / htMoonraker follow.
    htPrusaLink, htPrusaConnect, htOctoPrint, htDuet, htFlashAir, htAstroBox, htRepetier, htMKS, htESP3D, htCrealityPrint, htObico, htFlashforge, htSimplyPrint, htElegooLink, htMoonRaker_mqtt, htMoonRaker, ht3DPrinterOS, htMoonraker
};
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    // Snapmaker: per-extruder (coFloats in PrintConfig.cpp)
...
PRINT_CONFIG_CLASS_DEFINE(
    ...
    // Snapmaker: per-extruder (coBools/coPercents/coFloatsOrPercents in PrintConfig.cpp)
    ...
    // nozzle_volume_type: declared once, in the BBS block above (upstream added it there too)
...
PRINT_CONFIG_CLASS_DERIVED_DEFINE(
    ...
    // Snapmaker: filament_colour now lives in PrintConfig's base GCodeConfig (upstream moved it); the fork's
    // multi-colour keys stay here next to upstream's filament_multi_colour / filament_colour_type (defined in PrintConfig.cpp).
    ...
    // filament_tower_ironing_area: declared once, in GCodeConfig (Orca: mmu block; upstream added it there too)
