...
#define ORCA_CHECK_GCODE_PLACEHOLDERS 1
    ...
    auto process_flow_value(const VectorOption &opt) const -> decltype(opt.get_at(0))
    {
        return get_value_at(m_config, opt, ConfigFlowDomain::Process,
                            m_writer.filament() ? m_writer.filament()->id() : 0);
    }

    void print_machine_envelope(GCodeOutputStream& file, Print& print);
    // MERGE(2.4.2): upstream's signature is kept (first_printing_extruder_id feeds upstream's bed_temperature_formula);
    // the fork's max-over-all-used-filaments bed temperature must be applied inside the definition in GCode.cpp.
