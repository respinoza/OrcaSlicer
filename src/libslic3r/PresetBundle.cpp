...
#include "Model.hpp"
// Snapmaker: version macros (SLIC3R_VERSION, Snapmaker_VERSION, ...) come from common_func.hpp;
// the fork does not generate libslic3r_version.h, and both headers define the same macros.
...
std::vector<SmFilamentColor> BuildFilamentColors(const std::vector<std::string>& colors,
                                               const std::vector<std::string>& multiColors,
                                               const std::vector<FilamentColorMode>& modes, size_t targetCount)
{
    std::vector<SmFilamentColor> filamentColors;
    ...
    for (size_t i = 0; i < targetCount; ++i)
    {
        ...
        if (i < modes.size())
            colorMode = modes[i];
        filamentColors.emplace_back(SmFilamentColor::FromMultiColors(multiColor, colorMode, fallbackColor));
    }

    return filamentColors;
}

std::vector<SmFilamentColor> LoadFilamentColors(const AppConfig &config, const std::string &printerName, size_t targetCount)
{
...
void ApplyFilamentColors(DynamicPrintConfig &config, const std::vector<SmFilamentColor>& filamentColors)
{
    ...
    for (const SmFilamentColor& color : filamentColors)
    {
...
// Merge 2.4.2: upstream keeps its own multi-colour family next to the fork's one:
//   fork:     filament_multi_colors ("#A|#B" per filament), filament_colour_mode (0 = segment, 1 = gradient)
//   upstream: filament_multi_colour ("#A #B" per filament), filament_colour_type ("0" = gradient, "1" = default)
// Both describe the same data. The fork family is the one persisted in AppConfig (its "filament_multi_colors"
// printer setting predates upstream's key of the same name, which uses a different format), so the upstream
// family is re-derived from it, and the fork family is re-derived from upstream's where upstream code writes it.
void SyncUpstreamColourFieldsFromFork(DynamicPrintConfig &config)
{
    const ConfigOptionStrings *filamentColor = config.option<ConfigOptionStrings>("filament_colour");
    if (filamentColor == nullptr)
        return;

    EnsureFilamentColorFieldsAligned(config);
    const ConfigOptionStrings *multiColors = config.option<ConfigOptionStrings>("filament_multi_colors");
    const ConfigOptionInts    *modes       = config.option<ConfigOptionInts>("filament_colour_mode");
    ConfigOptionStrings *upstreamMultiColors = config.option<ConfigOptionStrings>("filament_multi_colour", true);
    ConfigOptionStrings *upstreamColourTypes = config.option<ConfigOptionStrings>("filament_colour_type", true);

    const size_t count = filamentColor->values.size();
    upstreamMultiColors->values.resize(count);
    upstreamColourTypes->values.resize(count);
    for (size_t i = 0; i < count; ++i)
    {
        const std::string multiColor = multiColors != nullptr && i < multiColors->values.size() ? multiColors->values[i] : std::string();
        const int modeValue = modes != nullptr && i < modes->values.size() ? modes->values[i] : 0;
        const SmFilamentColor color = SmFilamentColor::FromMultiColors(multiColor, FilamentColorModeFromConfig(modeValue),
                                                                   filamentColor->values[i]);
        upstreamMultiColors->values[i] = boost::algorithm::join(color.colors, " ");
        upstreamColourTypes->values[i] = color.IsGradient() ? "0" : "1";
    }
}

void SyncForkColourFieldsFromUpstream(DynamicPrintConfig &config)
{
    const ConfigOptionStrings *filamentColor = config.option<ConfigOptionStrings>("filament_colour");
    if (filamentColor == nullptr)
        return;

    const ConfigOptionStrings *upstreamMultiColors = config.option<ConfigOptionStrings>("filament_multi_colour");
    const ConfigOptionStrings *upstreamColourTypes = config.option<ConfigOptionStrings>("filament_colour_type");
    ConfigOptionStrings *multiColors = config.option<ConfigOptionStrings>("filament_multi_colors", true);
    ConfigOptionInts    *modes       = config.option<ConfigOptionInts>("filament_colour_mode", true);

    const size_t count = filamentColor->values.size();
    multiColors->values.resize(count);
    modes->values.resize(count);
    for (size_t i = 0; i < count; ++i)
    {
        std::vector<std::string> stops;
        if (upstreamMultiColors != nullptr && i < upstreamMultiColors->values.size() && !upstreamMultiColors->values[i].empty())
            boost::algorithm::split(stops, upstreamMultiColors->values[i], boost::algorithm::is_any_of(" "), boost::algorithm::token_compress_on);
        const bool gradient = upstreamColourTypes != nullptr && i < upstreamColourTypes->values.size() &&
                              upstreamColourTypes->values[i] == "0";
        const SmFilamentColor color = SmFilamentColor::FromColors(stops, gradient ? FilamentColorMode::Gradient : FilamentColorMode::Segment,
                                                              filamentColor->values[i]);
        multiColors->values[i] = color.ToMultiColorsString();
        modes->values[i] = FilamentColorModeToConfig(color.NormalizedMode());
    }
    EnsureFilamentColorFieldsAligned(config);
}

// Merge 2.4.2: upstream persists its filament_multi_colour family under the printer setting "filament_multi_colors",
// which is the fork's key (different format). Upstream's family therefore gets its own, distinct AppConfig key so it
// can never clobber the fork's one; the fork family stays authoritative.
const char *const UPSTREAM_MULTI_COLOUR_SETTING = "filament_multi_colour";

// Load the project colours of a printer from AppConfig: the fork family (filament_colors / filament_multi_colors /
// filament_colour_mode) wins; upstream's family (own key + "filament_color_types") is only used when the fork family
// was never persisted for this printer. Upstream's filament_multi_colour / filament_colour_type are re-derived.
void LoadProjectFilamentColours(const AppConfig &config, const std::string &printerName, size_t count, DynamicPrintConfig &project_config)
{
    std::vector<SmFilamentColor> filamentColors = LoadFilamentColors(config, printerName, count);
    ApplyFilamentColors(project_config, filamentColors);
    EnsureFilamentVolumeTypesAligned(project_config, count);
    if (!config.has_printer_setting(printerName, "filament_multi_colors") &&
        config.has_printer_setting(printerName, UPSTREAM_MULTI_COLOUR_SETTING)) {
        std::vector<std::string> upstreamMultiColors = SplitPrinterSetting(config, printerName, UPSTREAM_MULTI_COLOUR_SETTING);
        std::vector<std::string> upstreamColourTypes = SplitPrinterSetting(config, printerName, "filament_color_types");
        upstreamMultiColors.resize(count);
        upstreamColourTypes.resize(count, "1");
        project_config.option<ConfigOptionStrings>("filament_multi_colour", true)->values = upstreamMultiColors;
        project_config.option<ConfigOptionStrings>("filament_colour_type", true)->values  = upstreamColourTypes;
        SyncForkColourFieldsFromUpstream(project_config);
    }
    SyncUpstreamColourFieldsFromFork(project_config);
}

// Merge 2.4.2: AMS / machine tray configs carry their colour stops as "filament_multi_colour" (upstream) or
// "filament_multi_colors" (fork). Accept either, and never dereference a missing option.
std::vector<std::string> AmsTrayMultiColors(const DynamicPrintConfig &tray)
{
    if (const ConfigOptionStrings *opt = tray.option<ConfigOptionStrings>("filament_multi_colour"))
        return opt->values;
    if (const ConfigOptionStrings *opt = tray.option<ConfigOptionStrings>("filament_multi_colors"))
        return opt->values;
    return {};
}

// Merge 2.4.2: upstream collapses filament_options_with_variant to the value of the extruder variant a
// filament is mapped to. Presets that declare Snapmaker flow variants (filament_flow_support) keep those
// keys arrayed per flow variant instead (resolved at read time), so they must not be collapsed here.
std::set<std::string> FilamentExtruderVariantKeys(const DynamicPrintConfig &filament_config)
{
    std::set<std::string> keys = filament_options_with_variant;
    const auto *flow_support = filament_config.option<ConfigOptionStrings>("filament_flow_support");
...
static std::vector<std::string> s_project_options {
    ...
    // BBS
    "filament_colour",
    // Snapmaker: colour family of the fork
    "filament_multi_colors",
    "filament_colour_mode",
    // Orca 2.4: upstream colour family (kept in sync with the fork's one)
    ...
    // Orca 2.4 (nozzle_volume_type is listed above with the Snapmaker flow variants)
    "filament_map_mode",
    "filament_map"
};
...
// Orca: the generic "Custom" bundle. Snapmaker is the default bundle of this fork (SM_* above), but upstream
// code (apply_vendor_config, ConfigWizard, GCodeViewer) still refers to the Orca generic bundle by these names.
...
// MERGE-TODO(2.4.2): upstream-only composer (used by CalibUtils). Unlike full_fff_config() it does not apply the
// Snapmaker flow-variant segment layout (filament_flow_support / filament_flow_step_size), so calibration configs
// built from flow-variant filament presets carry upstream's plain concatenation of those keys.
...
std::pair<PresetsConfigSubstitutions, std::string> PresetBundle::load_system_presets_from_json(ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (!orca_lib_vendor.empty()) {
        ...
        if (startup_profile) {
            ...
            startup_profile_log("PresetBundle::load_system_presets_from_json vendor=" + orca_lib_vendor +
    ...
    // Snapmaker: per-vendor load time for ORCA_STARTUP_PROFILE (measured inside the parallel load).
    std::vector<long long>                          parallel_load_ms(other_vendors.size(), 0);
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, other_vendors.size()),
        [&](const tbb::blocked_range<size_t>& range) {
            for (size_t i = range.begin(); i < range.end(); ++i) {
                ...
                parallel_load_ms[i] = static_cast<long long>(
                    std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - vendor_start).count());
            }
        });
    ...
    for (size_t i = 0; i < other_vendors.size(); ++i) {
        if (startup_profile)
            startup_profile_log("PresetBundle::load_system_presets_from_json vendor=" + other_vendors[i] +
                                " vendor_ms=" + std::to_string(parallel_load_ms[i]));
...
void PresetBundle::update_selections(AppConfig &config)
{
    ...
    // Snapmaker: colours are persisted in the fork's colour family (filament_colors / filament_multi_colors /
    // filament_colour_mode printer settings); upstream's filament_multi_colour / filament_colour_type are derived
    // from it (upstream's family has its own AppConfig key, see UPSTREAM_MULTI_COLOUR_SETTING).
    LoadProjectFilamentColours(config, initial_printer_profile_name, filament_presets.size(), project_config);

    std::vector<int> filament_maps(filament_presets.size(), 1);
    ...
    } else if (config.has("app", "flush_multiplier")) {
        // Snapmaker Orca <= 2.4.0 stored one global scalar under [app] flush_multiplier; flush_multiplier is now a
        // per-nozzle vector stored per printer. Apply the legacy scalar to every nozzle until it is re-saved.
        const std::string str_flush_multiplier = config.get("app", "flush_multiplier");
        if (!str_flush_multiplier.empty()) {
            try {
                std::vector<double> &values = project_config.option<ConfigOptionFloats>("flush_multiplier")->values;
                values.assign(std::max<size_t>(values.size(), 1), boost::lexical_cast<double>(str_flush_multiplier));
...
void PresetBundle::load_selections(AppConfig &config, const PresetPreferences& preferred_selection/* = PresetPreferences()*/)
{
    ...
    if (selected_printer)
    {
        ...
        if (is_placeholder_filament_selection(initial_filament_profile_name) && !prefered_filament_profiles.empty()) {
            ...
            // If not visible, keep the placeholder (ORCA_DEFAULT_FILAMENT_PLACEHOLDER), which will be resolved later
        }
...
void PresetBundle::export_selections(AppConfig &config)
{
    ...
    const std::vector<SmFilamentColor> projectFilamentColors =
    ...
    for (const SmFilamentColor& color : projectFilamentColors)
    {
    ...
    // Merge 2.4.2: upstream writes filament_multi_colour into the "filament_multi_colors" printer setting, which is
    // the fork's key written above (different format). Upstream's family is stored under its own key instead, derived
    // from the fork family (authoritative) so the two can never disagree in AppConfig.
    {
        DynamicPrintConfig colourCfg;
        colourCfg.set_key_value("filament_colour", new ConfigOptionStrings(filamentColorValues));
        colourCfg.set_key_value("filament_multi_colors", new ConfigOptionStrings(filamentMultiColorValues));
        std::vector<int> modeValues;
        modeValues.reserve(projectFilamentColors.size());
        for (const SmFilamentColor& color : projectFilamentColors)
            modeValues.emplace_back(FilamentColorModeToConfig(color.NormalizedMode()));
        colourCfg.set_key_value("filament_colour_mode", new ConfigOptionInts(modeValues));
        SyncUpstreamColourFieldsFromFork(colourCfg);
        config.set_printer_setting(printer_name, UPSTREAM_MULTI_COLOUR_SETTING,
                                   boost::algorithm::join(colourCfg.option<ConfigOptionStrings>("filament_multi_colour")->values, ","));

        // Load filament color type data into app config
        std::string filament_color_types = boost::algorithm::join(colourCfg.option<ConfigOptionStrings>("filament_colour_type")->values, ",");
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(": printer %1%, print %2%, filaments[0] %3% ")%printers.get_selected_preset_name() % prints.get_selected_preset_name() %(filament_presets.empty() ? std::string("<none>") : filament_presets.front());
}
...
void PresetBundle::update_num_filaments(unsigned int to_del_filament_id)
{
    ...
    // update edited_preset
    {
        ...
        if (edited_preset_deleted && !filament_presets.empty()) {
    ...
    if (filament_color->values.size() > to_del_filament_id) {
        ...
        if (filament_map->values.size() > to_del_filament_id) {
            filament_map->values.erase(filament_map->values.begin() + to_del_filament_id);
        }
    }
    else {
        filament_color->values.resize(to_del_filament_id);
        filament_map->values.resize(to_del_filament_id, 1);
    }

    // lambda function to erase or resize the container
    auto erase_or_resize = [to_del_filament_id](auto& container) {
        if (container.size() > to_del_filament_id) {
            container.erase(container.begin() + to_del_filament_id);
        } else {
            container.resize(to_del_filament_id);
        }
    };
    ...
    // Snapmaker: keep the fork's colour family and the per-filament flow volume types aligned.
    ...
    // Merge 2.4.2: the fork family is authoritative; keep upstream's family in step with it.
    SyncUpstreamColourFieldsFromFork(project_config);
    // Snapmaker: pass the old count so the mixed-filament id remap is rebuilt for the deletion.
...
void PresetBundle::set_num_filaments(unsigned int n, std::vector<std::string> new_colors) {
    ...
    // Merge 2.4.2: upstream reset filament_multi_colour to the single colours above; the fork family is
    // authoritative, so re-derive upstream's family from it (keeps multi-colour filaments intact).
    SyncUpstreamColourFieldsFromFork(project_config);

    // Snapmaker: pass the old count so the mixed-filament id remap is rebuilt for the added filaments.
...
// Snapmaker: AMS / machine filament sync used by the fork's Sidebar::sync_ams_list(). Upstream's overload with
// mapping / append / merge support (below) is used by upstream's SyncAmsInfoDialog flow; both are kept.
unsigned int PresetBundle::sync_ams_list(unsigned int &unknowns)
{
    ...
    for (auto &entry : filament_ams_list) {
        ...
        std::vector<std::string> filament_multi_color = AmsTrayMultiColors(ams);
    ...
    EnsureFilamentColorFieldsAligned(project_config);
    EnsureFilamentVolumeTypesAligned(project_config, filament_presets.size());
    // Merge 2.4.2: keep upstream's per-filament arrays sized to the synced filament count.
    SyncUpstreamColourFieldsFromFork(project_config);
    project_config.option<ConfigOptionInts>("filament_map")->values.resize(filament_presets.size(), 1);
...
void PresetBundle::get_ams_cobox_infos(AMSComboInfo& combox_info)
{
    ...
    for (auto &entry : filament_ams_list) {
        ...
        auto  filament_changed     = !ams.has("filament_changed") || ams.opt_bool("filament_changed");
        auto  filament_multi_color = AmsTrayMultiColors(ams);
        if (filament_id.empty()) {
            continue;
        }
...
unsigned int PresetBundle::sync_ams_list(std::vector<std::pair<DynamicPrintConfig *,std::string>> &unknowns, bool use_map, std::map<int, AMSMapInfo> &maps, bool enable_append, MergeFilamentInfo &merge_info, bool color_only)
{
    ...
    // Snapmaker: mirror the synced colours into the fork's colour family and keep the per-filament
    // flow volume types aligned with the new filament count.
    SyncForkColourFieldsFromUpstream(project_config);
    EnsureFilamentVolumeTypesAligned(project_config, filament_presets.size());
    update_multi_material_filament_presets();
...
DynamicPrintConfig PresetBundle::full_fff_config(bool apply_extruder, std::optional<std::vector<int>> filament_maps_new) const
{
    ...
    if (num_filaments <= 1) {
        ...
        if (apply_extruder) {
            // Snapmaker: flow-variant keys of presets that declare filament_flow_support stay arrayed per flow variant.
            std::set<std::string> filament_variant_keys = FilamentExtruderVariantKeys(filament_config);
            filament_config.update_values_to_printer_extruders(out, filament_variant_keys, "", "filament_extruder_variant", 1, filament_maps[0]);
        }
        out.apply(filament_config);
        ...
        // surplus legacy values of presets without a flow_support declaration.
        {
            ...
            const bool has_flow_support = flow_support != nullptr && !flow_support->values.empty();
            int flow_step_size = 1;
            if (has_flow_support)
                flow_step_size = int(flow_support->values.size());

            // Merge 2.4.2: a preset without a flow_support declaration keeps upstream's per-extruder-variant
            // arrays when the variants are not applied (apply_extruder == false, e.g. full_config_secure()),
            // so only truncate it when the extruder variants were collapsed above.
            if (has_flow_support || apply_extruder) {
                for (const std::string &key : filament_flow_variant_options()) {
                    ConfigOption *opt_dst = out.option(key, false);
        ...
        for (size_t i = 0; i < num_filaments; ++i) {
            ...
            if (apply_extruder) {
                // Snapmaker: flow-variant keys of presets that declare filament_flow_support stay arrayed per flow variant.
                std::set<std::string> filament_variant_keys = FilamentExtruderVariantKeys(filament_temp_configs[i]);
                filament_temp_configs[i].update_values_to_printer_extruders(out, filament_variant_keys, "", "filament_extruder_variant", 1, filament_maps[i]);
            }
        }

        // Merge 2.4.2: the Snapmaker flow-variant segment layout is used for flow keys whenever a preset declares
        ...
        for (const t_config_option_key &key : this->filaments.default_preset().config.keys()) {
            ...
            if (opt_dst->is_scalar()) {
                ...
                if (opt_src != nullptr)
                    opt_dst->set(opt_src);
            } else if (use_flow_segments && (is_filament_flow_variant_option(key) || key == "filament_flow_support")) {
                ConfigOptionVectorBase* opt_vec_dst = static_cast<ConfigOptionVectorBase*>(opt_dst);
                opt_vec_dst->resize(flow_total_size);
                size_t segment_start = 0;
                for (size_t i = 0; i < num_filaments; ++i) {
                    const ConfigOption *opt_src = filament_temp_configs[i].option(key);
                ...
                {
                    if (apply_extruder) {
                        // Snapmaker: one value per filament, tolerating presets that lack the option or hold an empty vector.
        ...
        if (num_filaments <= 1) {
            ...
            for (const t_config_option_key &key : this->filaments.default_preset().config.keys()) {
                ...
                if (other_opt->is_scalar()) {
                    ...
                    // Snapmaker: flow-variant keys stored as per-filament segments (filament_flow_step_size table)
                    // are split back by segment; otherwise upstream's multi-extruder variant split applies.
                    ...
                    for (size_t i = 0; i < configs.size(); ++i) {
                        ...
                        if (uses_flow_variant_segment) {
                        ...
                        } else if (process_multi_extruder && (filament_options_with_variant.find(key) != filament_options_with_variant.end())) {
                            ...
                            dst->set(other_opt, filament_variant_index[i], next_index - filament_variant_index[i]);
                        } else {
...
void PresetBundle::update_multi_material_filament_presets(size_t to_delete_filament_id, size_t old_num_filaments_arg)
{
    ...
    // Snapmaker: capture the pre-change state for the mixed-filament id remap before the filament list is padded.
    ...
    // (virtual) entries from the physical set only.
    {
        ...
        if (color_opt) {
            ...
            // Merge 2.4.2: keep every per-filament project array the same length as filament_colour (the list may
            // just have been padded to the extruder count). Existing entries are kept; new ones follow their colour.
            EnsureFilamentColorFieldsAligned(this->project_config);
            if (ConfigOptionStrings *upstream_multi = this->project_config.option<ConfigOptionStrings>("filament_multi_colour", true)) {
                const size_t old_size = upstream_multi->values.size();
                upstream_multi->values.resize(num_filaments);
                for (size_t i = old_size; i < num_filaments; ++i)
                    upstream_multi->values[i] = color_opt->values[i];
            }
            if (ConfigOptionStrings *upstream_type = this->project_config.option<ConfigOptionStrings>("filament_colour_type", true))
                upstream_type->values.resize(num_filaments, "1");
            if (ConfigOptionInts *filament_map = this->project_config.option<ConfigOptionInts>("filament_map", true))
                filament_map->values.resize(num_filaments, 1);
            EnsureFilamentVolumeTypesAligned(this->project_config, num_filaments);
