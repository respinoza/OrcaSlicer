...
//    bool        	dont_adjust = false;
	...
	float role_speed = 0;
	// Filament-resolved speed overrides (may differ from role_speed on multi-filament setups).
	...
	for (size_t region_id = 0; region_id < layer.regions().size(); ++ region_id) {
	    ...
	    for (const Surface &surface : layerm.fill_surfaces.surfaces)
	        if (surface.surface_type == stInternalVoid)
				...
				// record speed params (Orca: filament-resolved speed overrides for multi-filament setups)
				// Note: computed unconditionally (not gated on params.bridge) because `params` is a single
				// object reused across surfaces in this loop; skipping the update for bridge-flow surfaces
				// left these fields holding a stale value from a previous surface, which role_speed below
				// would then pick up for a non-bridge role (e.g. sparse/solid infill patterns that use
				// bridge flow via Fill::use_bridge_flow()).
                {
				...
				// Infill speed setting for the effective extrusion role (feeds the g-code viewer's per-role speed display)
				params.role_speed = 0;
                if (params.extrusion_role == erBridgeInfill)
                    // bridge_speed is a per-extruder / flow-variant vector now: resolve it for this fill's filament.
                    params.role_speed = float(get_value_at(layer.object()->print()->config(), region_config.bridge_speed, ConfigFlowDomain::Process,
                                                           params.extruder > 0 ? params.extruder - 1 : 0));
                    ...
                    params.role_speed = params.sparse_infill_speed;
                else if (params.extrusion_role == erTopSolidInfill)
                    params.role_speed = params.top_surface_speed;
                else if (params.extrusion_role == erSolidInfill)
                    params.role_speed = params.solid_infill_speed;
		...
		if (! layerm->slices.empty()) {
			...
			if (ironing_params.extruder != -1) {
				...
				ironing_params.speed = (!config.filament_ironing_speed.is_nil(extruder_idx)
					...
					: config.ironing_speed.get_at(extruder_idx));
