...
const std::vector<std::string> filament_extruder_override_keys = {
    ...
    // Snapmaker: per-filament tool-change retraction overrides
...
void PrintConfigDef::init_fff_params()
{
    ...
    // Snapmaker: btGESP sits between btPCT and btSuperTack in BedType, so it must be listed here too,
    // otherwise index-based lookups (enum_values/enum_labels[bed_type - 1]) mis-map GESP and overrun on SuperTack.
    def->enum_values.emplace_back("Graphic Effect Plate");
    ...
    def->enum_labels.emplace_back(L("Graphic Effect Plate"));
    ...
    def->mode = comAdvanced;
    // Snapmaker: flow-variant vector option; upstream 2.4.2 default (false) kept.
    ...
    */
    ...
    // Upstream 2.4.2 dropped the duplicate first outer_wall_acceleration def (the later one wins);
    // the remaining def below stays vector-typed for the Snapmaker flow-variant presets.
    ...
    // "nozzle_volume_type" (one entry per extruder, listed in extruder_option_keys() so it is resized
    // together with nozzle_diameter by set_num_extruders()) is defined once, below, with upstream's
    // NozzleVolumeType keys ("Standard" / "High Flow"). Its integer values coincide with
    // FilamentVolumeType (0 = standard, 1 = high flow), so the flow-variant code keeps reading it as
    // FilamentVolumeType; projects saved by Snapmaker Orca <= 2.4.0 with the old "standard" /
    // "high_flow" strings are converted in handle_legacy().
...
//    def->min = 0;
    ...
    // filament_tower_ironing_area: the fork's own (comDevelop) def was merged into upstream's
    // definition next to the other filament_tower_interface_* options.
    ...
    // Snapmaker's extra keys (retract_length_toolchange, retract_restart_extra_toolchange) are
    // part of filament_extruder_override_keys at the top of this file.
...
void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
{
    ...
    if (opt_key == "curr_bed_type" && value == "SuperTack Plate") {
        ...
        if (opt_key == "nozzle_volume_type") {
            // Snapmaker Orca <= 2.4.0 serialized nozzle_volume_type with the flow-variant
            // names (FLOW_MODE_STANDARD / FLOW_MODE_HIGH_FLOW); same integer values.
            ReplaceString(value, FLOW_MODE_HIGH_FLOW, "High Flow");
            ReplaceString(value, FLOW_MODE_STANDARD, "Standard");
        }
    }
