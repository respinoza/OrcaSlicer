...
// Snapmaker: true for a key that is arrayed per declared flow variant (standard / high_flow ...) in a
// preset that actually declares more than one flow variant. Such keys keep the fork's plain differential
// save: upstream's set_with_nil() nil-marks entries equal to the parent by extruder-variant position,
// which does not describe the flow-variant layout.
static bool is_declared_flow_variant_option(const Preset &preset, const std::string &key)
{
    const char *support_key = preset_flow_support_key(preset.type);
    if (support_key == nullptr)
        return false;
    ...
    if (flow_support == nullptr || flow_support->values.size() <= 1)
        return false;
    switch (preset.type) {
    case Preset::TYPE_FILAMENT: return is_filament_flow_variant_option(key);
    case Preset::TYPE_PRINT:    return is_process_flow_variant_option(key);
    case Preset::TYPE_PRINTER:  return is_machine_flow_variant_option(key);
    ...
    if (config.option("filament_diameter") != nullptr) {
        ...
        for (const std::string &key : Preset::filament_options()) {
            ...
            if (opt != nullptr && opt->is_vector()) {
                const bool fork_flow_variant = is_filament_flow_variant_option(key);
                // Orca: extruder-variant options are sized per variant (see extend_default_config_length()),
                // not per filament; leave them alone unless the fork's flow-variant sizing owns the key.
                if (!fork_flow_variant && filament_options_with_variant.find(key) != filament_options_with_variant.end())
                    continue;
                const size_t expected_size = fork_flow_variant ? flow_variant_value_count : n;
    ...
    if (parent_config) {
        ...
        for (const std::string &option : dirty_options) {
            ...
            if (opt_dst->is_scalar() || !(opt_dst->nullable()))
                ...
                // Snapmaker: keys missing from the parent (added above via option_differs_from_default) have
                // nothing to nil-mark against; flow-variant keys keep the plain save.
                if (opt_vec_src->size() == 1 || opt_vec_inherit == nullptr || is_declared_flow_variant_option(*this, option))
                    opt_dst->set(opt_src);
...
static std::vector<std::string> s_Preset_printer_options {
    ...
    "cooling_tube_length", "high_current_on_filament_swap", "parking_pos_retraction", "extra_loading_move", "wipe_tower_type", "purge_in_prime_tower", "enable_filament_ramming", "tool_change_on_wipe_tower", "ramming_line_width_ratio", "enable_change_pressure_when_wiping", "ramming_pressure_advance_value",
    "z_offset",
    ...
    std::function<void(Preset&)> preset_loaded_fn, const PresetOrigin &load_origin)
{
    ...
    for (auto &dir_entry : boost::filesystem::directory_iterator(dir))
    {
        ...
        if (Slic3r::is_json_file(file_name)) {
            ...
            // Snapmaker: the "Preset already present" check runs below, once the logical name from the
            // JSON "name" key is known (a file name can differ from the preset name it holds).
            try {
        ...
        *new_preset = preset;
        ...
        for (const std::string &option : dirty_options) {
            ...
            if (opt_dst->is_scalar() || !(opt_dst->nullable()))
                ...
                // Snapmaker: keys missing from the parent (added above via option_differs_from_default) have
                // nothing to nil-mark against; flow-variant keys keep the plain save.
                if (opt_vec_src->size() == 1 || opt_vec_inherit == nullptr || is_declared_flow_variant_option(preset, option))
                    opt_dst->set(opt_src);
                else if (key_set1->find(option) != key_set1->end()) {
                    opt_vec_dst->set_with_nil(opt_vec_src, opt_vec_inherit, 1);
                }
    ...
    const std::string           filament_id)
{
    ...
    // Snapmaker: never copy a base value onto a key whose option type differs from the loaded one
    // (legacy/foreign project configs); comparing or setting mismatched option types throws.
    auto keys_with_same_type = [&keys, &cfg](const DynamicPrintConfig &base_config) {
        t_config_option_keys filtered;
        filtered.reserve(keys.size());
        for (const auto &opt : keys) {
            const ConfigOption *opt_base = base_config.option(opt);
            const ConfigOption *opt_cur  = cfg.option(opt);
            if (opt_base != nullptr && opt_cur != nullptr && opt_base->type() != opt_cur->type())
                continue;
            filtered.emplace_back(opt);
        }
        return filtered;
    };
    ...
    if (!inherits.empty() && (different_settings_list.size() > 0)) {
        ...
        if (iter != m_presets.end()) {
            ...
            cfg.update_non_diff_values_to_base_config(iter->config, keys_with_same_type(iter->config), different_settings_list, extruder_id_name, extruder_variant_name, *key_set1, *key_set2);
        }
    }
        ...
        cfg.update_non_diff_values_to_base_config(it->config, keys_with_same_type(it->config), different_settings_list, extruder_id_name, extruder_variant_name, *key_set1, *key_set2);
    }
...
bool PresetCollection::select_preset_by_name(const std::string &name_w_suffix, bool force)
{
    ...
    if (it != m_presets.end() && it->name == normalized_name && it->is_visible)
        ...
        if (m_type == Preset::Type::TYPE_PRINTER && it != m_presets.end() && it->name == normalized_name && !it->is_visible) {
