...
// Snapmaker: Snapmaker printers keep the fork's bed temperature rule whatever bed_temperature_formula says:
// the bed always runs the highest bed temperature of all filaments used by the print (PLA + TPU fix),
// see get_bed_temperature_max(). Other printers follow upstream's bed_temperature_formula.
static bool is_snapmaker_printer(const FullPrintConfig &config)
{
...
// Only add a newline in case the current G-code does not end with a newline.
    ...
    int OozePrevention::_get_temp(const GCode &gcodegen) const
    {
        ...
        return (gcodegen.layer() == nullptr || gcodegen.layer()->id() == 0
             || get_value_at(gcodegen.config(), gcodegen.config().nozzle_temperature, ConfigFlowDomain::Filament, gcodegen.writer().filament()->id()) == 0)
            ? get_value_at(gcodegen.config(), gcodegen.config().nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, gcodegen.writer().filament()->id())
            : get_value_at(gcodegen.config(), gcodegen.config().nozzle_temperature, ConfigFlowDomain::Filament, gcodegen.writer().filament()->id());
    }
    ...
    Wipe::RetractionValues Wipe::calculateWipeRetractionLengths(GCode& gcodegen, bool toolchange) {
        ...
        // Snapmaker: flow-variant values (wipe_distance, retraction_speed) are stored per filament,
        // so they are looked up by filament id (upstream indexed them by extruder->extruder_id()).
        auto filament_id = extruder->id();
        ...
        double wipe_dist = scale_(get_value_at(config, config.wipe_distance, ConfigFlowDomain::Filament, filament_id));
        ...
        retractionDuringWipe = get_value_at(config, config.retraction_speed, ConfigFlowDomain::Filament, filament_id) * unscale_(wipe_path_length) / wipe_speed;
    ...
    std::string Wipe::wipe(GCode& gcodegen,double length, bool toolchange, bool is_last)
    {
        ...
        if (length >= 0)
        {
            ...
            // BBS
            double wipe_dist = scale_(get_value_at(gcodegen.config(), gcodegen.config().wipe_distance, ConfigFlowDomain::Filament, gcodegen.writer().filament()->id()));
    ...
    static float get_outer_wall_volumetric_speed(const FullPrintConfig& config, const Print& print, int filament_id, int extruder_id) {
        ...
        float filament_max_volumetric_speed = get_value_at(config, config.filament_max_volumetric_speed, ConfigFlowDomain::Filament, filament_id);
        ...
        // Snapmaker: outer_wall_speed is a flow-variant process option
        float outer_wall_speed = get_value_at(config, print.default_region_config().outer_wall_speed, ConfigFlowDomain::Process, filament_id);
    ...
    std::string WipeTowerIntegration::append_tcr(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_filament_id, double z) const
    {
        ...
        ZHopType z_hope_type = ZHopType(get_value_at(gcodegen.config(), gcodegen.config().z_hop_types, ConfigFlowDomain::Filament, gcodegen.writer().filament()->id()));
        ...
        if (! change_filament_gcode.empty()) {
            ...
            // BBS
            {
                ...
                // Snapmaker: flow-variant filament values are read with get_value_at()
                float old_retract_length = (old_filament_id != -1) ? get_value_at(full_config, full_config.retraction_length, ConfigFlowDomain::Filament, old_filament_id) : 0;
                float new_retract_length = get_value_at(full_config, full_config.retraction_length, ConfigFlowDomain::Filament, new_filament_id);
                float old_retract_length_toolchange = (old_filament_id != -1) ? get_value_at(full_config, full_config.retract_length_toolchange, ConfigFlowDomain::Filament, old_filament_id) : 0;
                float new_retract_length_toolchange = get_value_at(full_config, full_config.retract_length_toolchange, ConfigFlowDomain::Filament, new_filament_id);
                int old_filament_temp = (old_filament_id != -1) ?
                    (gcodegen.on_first_layer() ? get_value_at(full_config, full_config.nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, old_filament_id) :
                                                 get_value_at(full_config, full_config.nozzle_temperature, ConfigFlowDomain::Filament, old_filament_id)) :
                    210;
                int new_filament_temp = gcodegen.on_first_layer() ? get_value_at(full_config, full_config.nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, new_filament_id) :
                                                                    get_value_at(full_config, full_config.nozzle_temperature, ConfigFlowDomain::Filament, new_filament_id);
                ...
                int old_filament_e_feedrate = (old_filament_id != -1) ? (int)(60.0 * get_value_at(full_config, full_config.filament_max_volumetric_speed, ConfigFlowDomain::Filament, old_filament_id) / filament_area) : 200;
                old_filament_e_feedrate = old_filament_e_feedrate == 0 ? 100 : old_filament_e_feedrate;
                int new_filament_e_feedrate = (int)(60.0 * get_value_at(full_config, full_config.filament_max_volumetric_speed, ConfigFlowDomain::Filament, new_filament_id) / filament_area);
                ...
                if (full_config.enable_tower_interface_features && tcr.is_contact) {
                    // Snapmaker: nozzle temperatures are flow-variant values (segmented per filament),
                    // flatten them to one value per filament before overriding (as done for the "temperature" placeholder).
                    const size_t     filament_count = full_config.filament_type.values.size();
                    std::vector<int> temps, first_layer_temps;
                    temps.reserve(filament_count);
                    first_layer_temps.reserve(filament_count);
                    for (size_t fid = 0; fid < filament_count; ++fid) {
                        temps.push_back(get_value_at(full_config, full_config.nozzle_temperature, ConfigFlowDomain::Filament, fid));
                        first_layer_temps.push_back(get_value_at(full_config, full_config.nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, fid));
                    }
                ...
                for (size_t idx = 0; idx < flush_v_speed.size(); ++idx) {
                    if (flush_v_speed[idx] == 0)
                        flush_v_speed[idx] = get_value_at(*m_print_config, m_print_config->filament_max_volumetric_speed, ConfigFlowDomain::Filament, idx);
                }
        ...
        if (get_value_at(gcodegen.config(), gcodegen.config().enable_pressure_advance, ConfigFlowDomain::Filament, new_filament_id)) {
            gcode += gcodegen.writer().set_pressure_advance(get_value_at(gcodegen.config(), gcodegen.config().pressure_advance, ConfigFlowDomain::Filament, new_filament_id));
            // Orca: Adaptive PA
            // Reset Adaptive PA processor last PA value
            gcodegen.m_pa_processor->resetPreviousPA(get_value_at(gcodegen.config(), gcodegen.config().pressure_advance, ConfigFlowDomain::Filament, new_filament_id));
        }
                                                  ...
                                                  double                             z) const
    {
        ...
        const bool should_travel_to_tower = !tcr.priming && (tcr.force_travel     // wipe tower says so
                                                             ...
                                                             // Snapmaker: no forced tower travel just because the tower layer is lower (will_go_down)
                                                             || is_ramming
        ...
        if (should_travel_to_tower || gcodegen.m_need_change_layer_lift_z) {
            ...
            auto type = ZHopType(get_value_at(gcodegen.m_config, gcodegen.m_config.z_hop_types, ConfigFlowDomain::Filament, gcodegen.m_writer.filament()->id()));
            if (type == ZHopType::zhtAuto) {
                type = ZHopType::zhtSpiral;
            }
            auto lift_type = gcodegen.to_lift_type(type);

            if (gcodegen.m_config.z_hop_when_prime.get_at(gcodegen.m_writer.filament()->id())) {
        ...
        if (toolchange_temp_override > 0) {
            int base_temp = gcodegen.on_first_layer() ? get_value_at(gcodegen.config(), gcodegen.config().nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, new_extruder_id)
                                                      : get_value_at(gcodegen.config(), gcodegen.config().nozzle_temperature, ConfigFlowDomain::Filament, new_extruder_id);
            if (std::abs(tcr.print_z) < EPSILON)
                base_temp = get_value_at(gcodegen.config(), gcodegen.config().nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, new_extruder_id);
    ...
    std::string WipeTowerIntegration::post_process_wipe_tower_moves(const WipeTower::ToolChangeResult& tcr, const Vec2f& translation, float angle) const
    {
        ...
        while (gcode_str) {
            ...
            if (line.find("G1 ") == 0 || line.find("G2 ") == 0 || line.find("G3 ") == 0) {
                ...
                // (Snapmaker's former "else line = line_out.str();" is covered by the unconditional strip above.)
            }
        ...
        auto emit_local_z_unplanned_toolchange = [&]() -> std::string {
            ...
            if (m_layer_idx >= 0 && size_t(m_layer_idx) < m_local_z_tool_changes.size() &&
                ...
                if (local_z_tool_change_idx < layer_local_z_tool_changes.size()) {
                    ...
                    if (local_z_tcr.new_tool == extruder_id &&
                        ...
                        return gcodegen.wipe_tower_type() == WipeTowerType::Type2 ? append_tcr2(gcodegen, local_z_tcr, extruder_id, tower_z) :
            ...
            if (gcodegen.m_curr_print != nullptr && gcodegen.writer().filament() != nullptr) {
                ...
                WipeTower2 local_z_wipe_tower(print_config,
                                              ...
                                              wipe_volumes,
                                              current_tool);
                // 2.4.x: nozzle_diameter is per physical extruder now; WipeTower2 tools are filaments (as in Print::_make_wipe_tower).
                for (size_t extruder_idx = 0; extruder_idx < print_config.filament_colour.values.size(); ++extruder_idx)
            ...
            const float nozzle_diameter = float(gcodegen.config().nozzle_diameter.get_at(gcodegen.get_extruder_id(unsigned(extruder_id))));
...
//        print_object_instances_ordering = sort_object_instances_by_max_z(print);
    ...
    // Snapmaker: nozzle temperatures are flow-variant values, flatten them to one value per filament for the placeholders.
    ...
    for (size_t idx = 0; idx < flush_v_speed.size(); ++idx) {
        if (flush_v_speed[idx] == 0)
            flush_v_speed[idx] = get_value_at(m_config, m_config.filament_max_volumetric_speed, ConfigFlowDomain::Filament, idx);
    }
    for (size_t idx = 0; idx < flush_temps.size(); ++idx) {
        if (flush_temps[idx] == 0)
    ...
    }
    {
        ...
        // Orca 2.4: bed_temperature_formula.
        // Snapmaker printers always keep the fork's behaviour: the single bed temperature accommodates the
        // highest-temperature filament of the whole print (max over all used extruders, not the initial one only).
        // Other printers follow upstream: "By highest temp" = max over the first-layer filaments, else the initial extruder.
        int target_bed_temp = 0;
        if (is_snapmaker_printer(m_config)) {
            for (const unsigned int extruder_id : print.extruders())
                target_bed_temp = std::max(target_bed_temp, first_bed_temp_opt->get_at(extruder_id));
        } else if (m_config.bed_temperature_formula == BedTempFormula::btfHighestTemp) {
            for (const unsigned int filament_id : print.get_slice_used_filaments(true))
                target_bed_temp = std::max(target_bed_temp, first_bed_temp_opt->get_at(filament_id));
        } else
            target_bed_temp = first_bed_temp_opt->get_at(initial_extruder_id);
    ...
    if (is_bbl_printers) {
        ...
        if (get_value_at(m_config, m_config.enable_pressure_advance, ConfigFlowDomain::Filament, initial_non_support_extruder_id)) {
            file.write(m_writer.set_pressure_advance(get_value_at(m_config, m_config.pressure_advance, ConfigFlowDomain::Filament, initial_non_support_extruder_id)));
            // Orca: Adaptive PA
            // Reset Adaptive PA processor last PA value
            m_pa_processor->resetPreviousPA(get_value_at(m_config, m_config.pressure_advance, ConfigFlowDomain::Filament, initial_non_support_extruder_id));
        }
    }
...
*/
    ...
    if (print.calib_params().mode == CalibMode::Calib_PA_Line) {
        ...
        if (outer_accel > 0) {
            gcode += m_writer.set_print_acceleration((unsigned int) floor(outer_accel + 0.5));
        }
...
DynamicConfig GCode::build_placeholder_process_config(unsigned int current_filament_id,
                                                       const DynamicConfig *config_override) const
{
    DynamicConfig process_config_override;
    const size_t process_config_idx = get_config_idx(m_config, ConfigFlowDomain::Process, current_filament_id);
...
PlaceholderParserIntegration &ppi = m_placeholder_parser_integration;
    DynamicConfig process_config_override =
        this->build_placeholder_process_config(current_filament_id, config_override);

    try {
        ppi.update_from_gcodewriter(m_writer);
        std::string output = ppi.parser.process(templ, current_filament_id, &process_config_override, &ppi.output_config, &ppi.context);
...
void GCode::_print_first_layer_bed_temperature(GCodeOutputStream &file, Print &print, const std::string &gcode, unsigned int first_printing_extruder_id, bool wait)
{
    ...
    int bed_temp = 0;
    if (is_snapmaker_printer(m_config)) {
        // Snapmaker: bed temperature accommodates the highest-temperature filament of the print
        // (max over all used extruders, including support/wipe tower/brim-introduced ones), whatever the formula.
        bed_temp = get_bed_temperature_max(print, true);
    }
    else if (m_config.bed_temperature_formula.value == BedTempFormula::btfHighestTemp) {
...
// Snapmaker: Local-Z clipping works in XY, which would drop the per-point Z offsets of Z-contoured (ZAA) paths.
// Give every clipped point the Z offset of the source path at that XY position: exact for the source vertices,
// linearly interpolated along the source segment for the points inserted by the clip.
static Polyline3 local_z_polyline_with_source_z(const Polyline& clipped, const Polyline3& src)
{
    Polyline3 out(clipped);
    const Points3& sp = src.points;
    if (sp.empty())
        return out;
    if (sp.size() == 1) {
        for (Point3& p : out.points)
            p.z() = sp.front().z();
        return out;
    }
    const size_t num_segments = sp.size() - 1;
    // The clipped points lie on the source polyline (up to Clipper's integer rounding) and mostly follow its order:
    // resume the search at the last matching segment and stop at the first segment that is close enough.
    const double on_segment_d2 = 16.; // (4 scaled units)^2
    size_t       hint          = 0;
    for (Point3& p : out.points) {
        const Vec2d pt(double(p.x()), double(p.y()));
        double      best_d2  = std::numeric_limits<double>::max();
        double      best_z   = double(sp.front().z());
        size_t      best_seg = hint;
        for (size_t k = 0; k < num_segments; ++k) {
            const size_t seg = (hint + k) % num_segments;
            const Vec2d  a(double(sp[seg].x()), double(sp[seg].y()));
            const Vec2d  b(double(sp[seg + 1].x()), double(sp[seg + 1].y()));
            const Vec2d  ab   = b - a;
            const double len2 = ab.squaredNorm();
            const double t    = len2 > 0. ? std::clamp((pt - a).dot(ab) / len2, 0., 1.) : 0.;
            const double d2   = (a + t * ab - pt).squaredNorm();
            if (d2 < best_d2) {
                best_d2  = d2;
                best_z   = double(sp[seg].z()) + t * double(sp[seg + 1].z() - sp[seg].z());
                best_seg = seg;
                if (d2 <= on_segment_d2)
                    break;
            }
        }
        p.z() = coord_t(std::round(best_z));
        hint  = best_seg;
    }
    return out;
}
...
{
    // Upstream 2.4 extrusion paths are 3D (Polyline3); the Local-Z clipping works in XY, like upstream's own clipping
    // (ExtrusionPath::intersect_expolygons()), so the clipped fragments start from the XY polyline. The per-point Z
    // offsets of Z-contoured (ZAA) paths are carried over to the fragments by local_z_polyline_with_source_z().
    // (ExtrusionPathContoured::z_diffs is not produced anywhere in this tree; a contoured source is copied as a plain
    // ExtrusionPath with its z_contoured flag, like before the merge.)
    const bool has_z = std::any_of(src_path.polyline.points.begin(), src_path.polyline.points.end(),
                                   [](const Point3& p) { return p.z() != 0; });
    ...
    for (Polyline& segment : segments) {
        if (!segment.is_valid())
            continue;
        ExtrusionPath clipped(has_z ? local_z_polyline_with_source_z(segment, src_path.polyline) : Polyline3(segment), src_path);
    ...
    const bool                               prime_extruder)
{
    ...
    ZHopType z_hope_type = ZHopType(get_value_at(m_config, m_config.z_hop_types, ConfigFlowDomain::Filament, m_writer.filament()->id()));
    LiftType auto_lift_type = LiftType::NormalLift;
    if (z_hope_type == ZHopType::zhtAuto || z_hope_type == ZHopType::zhtSpiral || z_hope_type == ZHopType::zhtSlope)
        auto_lift_type = LiftType::SpiralLift;
    ...
    if (!first_layer && !m_second_layer_things_done) {
      ...
      if (this->process_flow_value(m_config.default_jerk) > 0 && this->process_flow_value(m_config.initial_layer_jerk) > 0) {
        ...
        if (is_snapmaker_printer(m_config))
            // Snapmaker: bed temperature accommodates the highest-temperature filament of the print
            // (max over all used extruders instead of the first extruder of this layer only), whatever the formula.
            bed_temp = get_bed_temperature_max(print, false);
        else if (m_config.bed_temperature_formula == BedTempFormula::btfHighestTemp)
    ...
    // Snapmaker: configured 1-based filament id (possibly a mixed / Full Spectrum filament) of an island's extrusions.
    // Upstream 2.4 split the old wall / solid infill filament keys into outer / inner wall and internal solid /
    // top surface / bottom surface ids; they are routed the same way as in LayerTools::extruder().
    auto configured_filament_id_1based = [&layer_tools](const GCode::ObjectByExtruder::Island::Region::Type entity_type,
        ...
        if (entity_type == GCode::ObjectByExtruder::Island::Region::INFILL) {
            ...
            if (is_solid_infill(role)) {
                const ExtrusionRole collection_role = entities.role();
                if (collection_role == erTopSolidInfill || collection_role == erIroning)
                    return unsigned(region.config().top_surface_filament_id.value);
                if (collection_role == erBottomSurface)
        ...
        if (layer_tools.extruder_override != 0)
            return layer_tools.extruder_override;
        return unsigned(entities.role() == erPerimeter ? region.config().inner_wall_filament_id.value :
    ...
    auto configured_extruder_id = [&layer_tools](const GCode::ObjectByExtruder::Island::Region::Type entity_type,
        ...
        if (entity_type == GCode::ObjectByExtruder::Island::Region::INFILL) {
            ...
            if (is_solid_infill(role))
                // Internal solid / top surface / bottom surface filament, resolved through the mixed-filament manager.
                return int(layer_tools.extruder(entities, region));
            return int(layer_tools.sparse_infill_filament_id(region));
        }
        // Outer wall filament, or the inner wall filament for collections of inner walls only.
        return int(layer_tools.extruder(entities, region));
    };
            ...
            for (size_t region_id = 0; region_id < layer.regions().size(); ++ region_id) {
                ...
                for (const ObjectByExtruder::Island::Region::Type entity_type : { ObjectByExtruder::Island::Region::INFILL, ObjectByExtruder::Island::Region::PERIMETERS }) {
                    for (const ExtrusionEntity *ee : (entity_type == ObjectByExtruder::Island::Region::INFILL) ? layerm->fills.entities : layerm->perimeters.entities) {
                        ...
                        // Upstream 2.4: the island assignment runs per (possibly split) collection, so that walls with
                        // different outer / inner wall filaments can be split below. Snapmaker: the fork's grouped manual
                        // pattern split of multi-perimeter mixed filaments runs per collection as well.
    ...
    // Snapmaker: extruders printing on this layer, including the physical extruders introduced by mixed-filament
    // splits (pointillism / grouped manual patterns / Local-Z) that are not listed in layer_tools.extruders.
    ...
    {
        for (unsigned int filament_id : layer_extruders) {
...
std::string GCode::change_layer(coordf_t print_z)
{
    ...
    if (FILAMENT_CONFIG(retract_when_changing_layer) && m_writer.will_move_z(z)) {
        LiftType lift_type = this->to_lift_type(ZHopType(get_value_at(m_config, m_config.z_hop_types, ConfigFlowDomain::Filament, m_writer.filament()->id())));
        //BBS: force to use SpiralLift when change layer if lift type is auto
        gcode += this->retract(false, false, ZHopType(get_value_at(m_config, m_config.z_hop_types, ConfigFlowDomain::Filament, m_writer.filament()->id())) == ZHopType::zhtAuto ? LiftType::SpiralLift : lift_type);
    }
...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed)
{
    ...
    // Snapmaker: flow variant -- filament_max_volumetric_speed is a flow-variant filament option.
    // filament_max_volumetric_speed_cfg is the configured cap (upstream's FILAMENT_CONFIG(filament_max_volumetric_speed));
    // filament_max_volumetric_speed additionally applies upstream's adaptive volumetric speed fit.
    const double filament_max_volumetric_speed_cfg = get_value_at(m_config, m_config.filament_max_volumetric_speed,
                                                                  ConfigFlowDomain::Filament, m_writer.filament()->id());
    //BBS: if not set the speed, then use the filament_max_volumetric_speed directly
    double filament_max_volumetric_speed = filament_max_volumetric_speed_cfg;
    ...
    if (this->on_first_layer() || object_layer_over_raft()) {
        ...
        if (path.role() != erBottomSurface) {
            speed = is_perimeter(path.role()) ? this->process_flow_value(m_config.initial_layer_speed) :
                                                this->process_flow_value(m_config.initial_layer_infill_speed);
        }
    } else if (slow_down_layers > 1 && m_config.raft_layers == 0) {
        ...
        if (_layer > m_config.raft_layers && (_layer - m_config.raft_layers) < slow_down_layers) {
            const auto first_layer_speed = is_perimeter(path.role()) ? this->process_flow_value(m_config.initial_layer_speed) :
                                                                       this->process_flow_value(m_config.initial_layer_infill_speed);
            if (first_layer_speed < speed) {
                speed = std::min(speed, Slic3r::lerp(first_layer_speed, speed,
                                                     (double) (_layer - m_config.raft_layers) / slow_down_layers));
            }
        }
    }
    ...
    //    );
    //}
    if (filament_max_volumetric_speed_cfg > 0) {
        // cap speed with max_volumetric_speed anyway (even if user is not using autospeed)
        speed = std::min(speed, filament_max_volumetric_speed_cfg / _mm3_per_mm);
    }
    // ORCA: resonance‑avoidance on short external perimeters
{
    ...
    if (this->process_flow_value(m_config.enable_overhang_speed) && !this->on_first_layer() && !object_layer_over_raft() &&
        (is_bridge(path.role()) || is_perimeter(path.role()))) {
            ...
            if (ref_speed == 0)
                ref_speed = filament_max_volumetric_speed_cfg / _mm3_per_mm;

            if (filament_max_volumetric_speed_cfg > 0) {
                ref_speed = std::min(ref_speed, filament_max_volumetric_speed_cfg / _mm3_per_mm);
            }
            if (sloped) {
    ...
    if (!is_pa_calib && FILAMENT_CONFIG(adaptive_pressure_advance) && get_value_at(m_config, m_config.enable_pressure_advance, ConfigFlowDomain::Filament, m_writer.filament()->id())) {
    ...
    if (!variable_speed) {
        ...
        if( (std::abs(writer().get_current_speed() - F) > EPSILON) || (std::abs(_mm3_per_mm - m_last_mm3_mm) > EPSILON) ){
            ...
            if(_mm3_per_mm >0 &&
               ...
               get_value_at(m_config, m_config.enable_pressure_advance, ConfigFlowDomain::Filament, m_writer.filament()->id()) &&
...
std::string GCode::travel_to(const Point& point, ExtrusionRole role, std::string comment, double z/* = DBL_MAX*/)
{
    ...
    if (this->on_first_layer()) {
        // Snapmaker: flow variant -- the ratio_over bases (travel_acceleration / travel_jerk) are flow-variant
        // vector options, so resolve the percentages against the active variant by hand.
        unsigned int initial_layer_travel_acceleration =
            m_config.initial_layer_travel_acceleration.get_abs_value(this->process_flow_value(m_config.travel_acceleration));
        double initial_layer_travel_jerk = m_config.initial_layer_travel_jerk.get_abs_value(this->process_flow_value(m_config.travel_jerk));
    
        if (this->process_flow_value(m_config.default_acceleration) > 0 && initial_layer_travel_acceleration > 0) {
            acceleration_to_set = (unsigned int) floor(initial_layer_travel_acceleration + 0.5);
        }
        if (this->process_flow_value(m_config.default_jerk) > 0 && initial_layer_travel_jerk > 0) {
        ...
        if (this->process_flow_value(m_config.default_acceleration) > 0) {
            if (role == erOverhangPerimeter && is_short_travel) {
                // Snapmaker: flow variant -- bridge_acceleration is a FloatsOrPercents vector (percent of outer wall acceleration)
                const auto   bridge_fop          = this->process_flow_value(m_config.bridge_acceleration);
                const double bridge_acceleration = bridge_fop.percent ?
                                                       (bridge_fop.value * 0.01 * this->process_flow_value(m_config.outer_wall_acceleration)) :
                                                       bridge_fop.value;
...
bool GCode::needs_retraction(const Polyline &travel, ExtrusionRole role, LiftType& lift_type)
{
    ...
    if (is_perimeter(m_last_processor_extrusion_role) && m_last_processor_extrusion_role != erPerimeter) {
        if (ZHopType(get_value_at(m_config, m_config.z_hop_types, ConfigFlowDomain::Filament, m_writer.filament()->id())) == ZHopType::zhtAuto) {
            lift_type = is_through_overhang(clipped_travel) ? LiftType::SpiralLift : LiftType::SlopeLift;
        }
        else {
            lift_type = to_lift_type(ZHopType(get_value_at(m_config, m_config.z_hop_types, ConfigFlowDomain::Filament, m_writer.filament()->id())));
        }
        return true;
    }
...
std::string GCode::set_extruder(unsigned int new_filament_id, double print_z, bool by_object, int toolchange_temp_override)
{
    ...
    if (!m_writer.multiple_extruders) {
        ...
        if (get_value_at(m_config, m_config.enable_pressure_advance, ConfigFlowDomain::Filament, new_filament_id)) {
            gcode += m_writer.set_pressure_advance(get_value_at(m_config, m_config.pressure_advance, ConfigFlowDomain::Filament, new_filament_id));
            // Orca: Adaptive PA
            // Reset Adaptive PA processor last PA value
            m_pa_processor->resetPreviousPA(get_value_at(m_config, m_config.pressure_advance, ConfigFlowDomain::Filament, new_filament_id));
        }
    ...
    // BBS
    // Snapmaker: flow variant -- flow-variant filament options are read with get_value_at()
    float new_retract_length            = get_value_at(m_config, m_config.retraction_length, ConfigFlowDomain::Filament, new_filament_id);
    float new_retract_length_toolchange = get_value_at(m_config, m_config.retract_length_toolchange, ConfigFlowDomain::Filament, new_filament_id);
    int   new_filament_temp             = this->on_first_layer() ? get_value_at(m_config, m_config.nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, new_filament_id) :
                                                                   get_value_at(m_config, m_config.nozzle_temperature, ConfigFlowDomain::Filament, new_filament_id);
    // BBS: if print_z == 0 use first layer temperature
    if (abs(print_z) < EPSILON)
        new_filament_temp = get_value_at(m_config, m_config.nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, new_filament_id);
    if (toolchange_temp_override > 0)
        new_filament_temp = toolchange_temp_override;
    // Snapmaker: nozzle temperatures are flow-variant values (segmented per filament), flatten them to one
    // value per filament before applying toolchange_temp_override (as done for the wipe tower toolchange).
    auto flattened_nozzle_temps = [this](const ConfigOptionInts &opt) {
        const size_t     filament_count = m_config.filament_type.values.size();
        std::vector<int> temps;
        temps.reserve(filament_count);
        for (size_t fid = 0; fid < filament_count; ++fid)
            temps.push_back(get_value_at(m_config, opt, ConfigFlowDomain::Filament, fid));
        return temps;
    };
    ...
    if (m_writer.filament() != nullptr || m_start_gcode_filament != -1) {
        ...
        old_retract_length            = get_value_at(m_config, m_config.retraction_length, ConfigFlowDomain::Filament, old_filament_id);
        old_retract_length_toolchange = get_value_at(m_config, m_config.retract_length_toolchange, ConfigFlowDomain::Filament, old_filament_id);
        old_filament_temp             = this->on_first_layer() ? get_value_at(m_config, m_config.nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, old_filament_id) :
                                                                 get_value_at(m_config, m_config.nozzle_temperature, ConfigFlowDomain::Filament, old_filament_id);
        ...
        old_filament_e_feedrate = (int) (60.0 * get_value_at(m_config, m_config.filament_max_volumetric_speed, ConfigFlowDomain::Filament, old_filament_id) / filament_area);
        old_filament_e_feedrate = old_filament_e_feedrate == 0 ? 100 : old_filament_e_feedrate;
    ...
    int new_filament_e_feedrate = (int)(60.0 * get_value_at(m_config, m_config.filament_max_volumetric_speed, ConfigFlowDomain::Filament, new_filament_id) / filament_area);
    new_filament_e_feedrate = new_filament_e_feedrate == 0 ? 100 : new_filament_e_feedrate;
    ...
    if (toolchange_temp_override > 0) {
        auto temps = flattened_nozzle_temps(m_config.nozzle_temperature);
        ...
        auto first_layer_temps = flattened_nozzle_temps(m_config.nozzle_temperature_initial_layer);
    ...
    for (size_t idx = 0; idx < flush_v_speed.size(); ++idx) {
        if (flush_v_speed[idx] == 0)
            flush_v_speed[idx] = get_value_at(m_print->config(), m_print->config().filament_max_volumetric_speed, ConfigFlowDomain::Filament, idx);
    }
    for (size_t idx = 0; idx < flush_temps.size(); ++idx) {
        if (flush_temps[idx] == 0)
    ...
    if (m_config.single_extruder_multi_material && !m_config.enable_prime_tower) {
        int temp = (m_layer_index <= 0 ? get_value_at(m_config, m_config.nozzle_temperature_initial_layer, ConfigFlowDomain::Filament, new_filament_id) :
                                         get_value_at(m_config, m_config.nozzle_temperature, ConfigFlowDomain::Filament, new_filament_id));
