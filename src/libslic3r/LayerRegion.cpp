...
void LayerRegion::make_perimeters(const SurfaceCollection &slices, const LayerRegionPtrs &compatible_regions, SurfaceCollection* fill_surfaces, ExPolygons* fill_no_overlap)
{
    ...
    // Snapmaker: remap mixed/virtual filament ids to this layer's physical filament.
    // The fork's single wall filament maps to both wall ids; its solid infill filament to internal-solid/top/bottom surface.
    auto remap_filament = [this](ConfigOptionInt &opt, const ConfigOptionInt &src) {
        opt.value = int(effective_layer_filament_id(*this->layer(), unsigned(std::max(0, src.value))));
    };
    remap_filament(perimeter_config.outer_wall_filament_id,     region_config.outer_wall_filament_id);
    remap_filament(perimeter_config.inner_wall_filament_id,     region_config.inner_wall_filament_id);
    remap_filament(perimeter_config.sparse_infill_filament_id,  region_config.sparse_infill_filament_id);
    remap_filament(perimeter_config.internal_solid_filament_id, region_config.internal_solid_filament_id);
    remap_filament(perimeter_config.top_surface_filament_id,    region_config.top_surface_filament_id);
    remap_filament(perimeter_config.bottom_surface_filament_id, region_config.bottom_surface_filament_id);
