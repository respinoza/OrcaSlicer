...
// Snapmaker: resolve a configured 1-based filament id (possibly a mixed / Full Spectrum filament) to the
// physical 1-based extruder used on this layer. Same as LayerTools::resolve_mixed_1based(), usable from free code.
unsigned int resolve_layer_filament_1based(const LayerTools &layer_tools, unsigned int filament_id_1based)
{
...
// Snapmaker: infill-like roles (sparse / internal solid / top / bottom surface) honour grouped manual patterns first.
unsigned int resolve_infill_filament_1based(const LayerTools &layer_tools, const PrintRegion &region, unsigned int filament_id_1based)
{
    const unsigned int grouped = grouped_manual_pattern_infill_filament_1based(layer_tools, region, filament_id_1based);
    return (grouped != 0) ? grouped : resolve_layer_filament_1based(layer_tools, filament_id_1based);
}
...
// Snapmaker: the configured id may be a mixed filament; it is resolved to the physical extruder of this layer.
...
unsigned int LayerTools::extruder(const ExtrusionEntityCollection &extrusions, const PrintRegion &region) const
{
    ...
    // Snapmaker: every configured id (and extruder_override) is resolved through the mixed-filament manager
    // (Full Spectrum) by the accessors above / resolve_infill_filament_1based(); upstream's per-surface
    // filament ids (top / bottom surface, inner wall) are honoured when no override is active.
    if (extrusions.has_infill()) {
        const ExtrusionRole front_role = extrusions.entities.empty() ? erNone : extrusions.entities.front()->role();
        if (internal_solid_infill_uses_sparse_filament(region, front_role))
            return sparse_infill_filament_id(region);
        if (extrusions.has_solid_infill()) {
            const ExtrusionRole role = extrusions.role();
            if (this->extruder_override == 0 && (role == erTopSolidInfill || role == erIroning))
                return resolve_infill_filament_1based(*this, region, region.config().top_surface_filament_id.value) - 1;
            if (this->extruder_override == 0 && role == erBottomSurface)
                return resolve_infill_filament_1based(*this, region, region.config().bottom_surface_filament_id.value) - 1;
            return internal_solid_filament_id(region);
        }
        return sparse_infill_filament_id(region);
    }
    if (this->extruder_override == 0 && extrusions.role() == erPerimeter)
        return resolve_mixed_1based(region.config().inner_wall_filament_id.value) - 1;
...
void ToolOrdering::handle_dontcare_extruder(const std::vector<unsigned int>& tool_order_layer0)
{
    ...
    // Reorder the extruders of first layer
    {
        ...
        // Snapmaker: grouped manual mixed patterns fix the perimeter order of the layer.
...
ToolOrdering::ToolOrdering(const PrintObject &object, unsigned int first_extruder, bool prime_multi_material)
{
    ...
    // Snapmaker (mixed filament): the print config is also available in by-object mode.
...
ToolOrdering::ToolOrdering(const Print &print, unsigned int first_extruder, bool prime_multi_material)
{
	...
	if (auto num_physical = unsigned(print.config().filament_diameter.size());
        ...
        // Snapmaker: custom tool changes may target mixed filaments (ids above the physical count).
...
void ToolOrdering::collect_extruders(const PrintObject &object, const std::vector<std::pair<double, unsigned int>> &per_layer_extruder_switches)
{
        ...
        for (const LayerRegion *layerm : layer->regions()) {
            ...
            if (! layerm->perimeters.entities.empty()) {
                ...
                if (something_nonoverriddable){
                    // Snapmaker: wall filaments may be mixed (Full Spectrum) filaments. They are resolved to the
                    // physical extruder of this layer; grouped manual patterns add every perimeter extruder in
                    // print order and pin that order for the layer.
                    auto append_wall_filament = [&](unsigned int configured_wall, bool record_first_layer) {
                        const unsigned int wall_ext   = resolve_mixed(configured_wall, layerCount, float(layer->print_z), float(layer->height), &object);
                        const unsigned int grouped_id = grouped_manual_pattern_mixed_filament_id_for_layer(layer_tools, configured_wall);
                        std::vector<unsigned int> ordered;
                        if (grouped_id != 0)
                            ordered = m_mixed_mgr->ordered_perimeter_extruders(grouped_id,
                                                                               m_num_physical,
                                                                               layerCount,
                                                                               float(layer->print_z),
                                                                               float(layer->height));
                        if (!ordered.empty()) {
                            ...
                            for (unsigned int extruder_id : ordered) {
                                ...
                                if (record_first_layer && layerCount == 0 &&
                    ...
                    append_wall_filament(configured_wall, true);
                    if (extruder_override == 0 && region.config().wall_loops.value > 1 &&
                        unsigned(region.config().inner_wall_filament_id.value) != configured_wall)
                        append_wall_filament(region.config().inner_wall_filament_id.value, false);
                }
            ...
            for (const ExtrusionEntity *ee : layerm->fills.entities) {
                ...
                else if (internal_solid_infill_uses_sparse_filament(region, role))
                    // Snapmaker: internal solid infill of a 100 % sparse region prints with the sparse filament.
                    has_infill = true;
            ...
            if (something_nonoverriddable || !m_print_config_ptr) {
            	if (extruder_override == 0) {
                    // Snapmaker: resolve mixed (Full Spectrum) filaments to this layer's physical extruders.
                    ...
                    if (has_top_solid_surface)
                        layer_tools.extruders.emplace_back(resolve_infill_filament_1based(layer_tools, region, region.config().top_surface_filament_id.value));
                    if (has_bottom_surface)
                        layer_tools.extruders.emplace_back(resolve_infill_filament_1based(layer_tools, region, region.config().bottom_surface_filament_id.value));
	                if (has_infill)
    ...
    for (auto support_layer : object.support_layers()) {
        ...
        // Snapmaker: supports are now collected after the object layers (upstream order); keep the object layer
        // height (used by mixed-filament resolution) where an object layer shares this print_z, as the fork did.
        ...
        // Snapmaker: support filaments may be mixed (Full Spectrum) filaments; 0 ("don't care") passes through.
    ...
    for (auto& layer : m_layer_tools) {
        // Sort and remove duplicates (Snapmaker: keep the pinned order of grouped mixed patterns)
...
void ToolOrdering::reorder_extruders_for_minimum_flush_volume(bool reorder_first_layer)
{
    ...
    // other_layers_seq: the layer_idx and extruder_idx are base on 1
    auto get_custom_seq = [this, &other_layers_seqs, &reorder_first_layer, &first_layer_filaments](int layer_idx, std::vector<int>& out_seq) -> bool {
        // Snapmaker: layers whose extruder order is pinned by a grouped manual mixed pattern keep that order.
        if (layer_idx >= 0 && size_t(layer_idx) < m_layer_tools.size() && m_layer_tools[layer_idx].preserve_extruder_order &&
            !m_layer_tools[layer_idx].extruders.empty()) {
            const auto &pinned = m_layer_tools[layer_idx].extruders;
            out_seq.resize(pinned.size());
            std::transform(pinned.begin(), pinned.end(), out_seq.begin(), [](auto item) { return int(item) + 1; });
            return true;
        }
