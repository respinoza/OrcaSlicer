...
namespace Slic3r
{
        ...
        float* cost)
    {
        ...
        // Snapmaker: guard against filament ids outside the flush matrix (e.g. mixed / virtual filament ids that were not
        // resolved to physical filaments): keep the given order instead of indexing past wipe_volumes.
        {
            const auto outside_matrix = [&wipe_volumes](unsigned int id) { return id >= wipe_volumes.size(); };
            if (std::any_of(curr_layer_extruders.begin(), curr_layer_extruders.end(), outside_matrix) ||
                (start_extruder_id && outside_matrix(*start_extruder_id)) ||
                (use_forcast && std::any_of(next_layer_extruders.begin(), next_layer_extruders.end(), outside_matrix))) {
                if (cost)
                    *cost = 0;
                return curr_layer_extruders;
            }
