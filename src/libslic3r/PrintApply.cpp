        ...
        for (unsigned int painted_extruder_id : painting_extruders)
            for (int parent_region_id = 0; parent_region_id < int(layer_range.volume_regions.size()); ++ parent_region_id)
                if (const PrintObjectRegions::VolumeRegion &parent_region = layer_range.volume_regions[parent_region_id];
                    ...
                    if (painted_region->config().outer_wall_filament_id.value != int(painted_extruder_id) ||
                        painted_region->config().inner_wall_filament_id.value != int(painted_extruder_id) ||
                        painted_region->config().internal_solid_filament_id.value != int(painted_extruder_id) ||
                        painted_region->config().top_surface_filament_id.value != int(painted_extruder_id) ||
                        painted_region->config().bottom_surface_filament_id.value != int(painted_extruder_id) ||
                        painted_region->config().sparse_infill_filament_id.value != int(painted_extruder_id)) {
...
Print::ApplyStatus Print::apply(const Model &model, DynamicPrintConfig new_full_config, bool extruder_applied)
{
    ...
    if (! full_config_diff.empty()) {
        ...
        m_placeholder_parser.set("version", std::string(Snapmaker_VERSION)); // Snapmaker: same value PlaceholderParser's constructor sets
