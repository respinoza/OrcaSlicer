                ...
                while(!finished)
                {
                    ...
                    for (int index = 0; index < partplate_list.get_plate_count(); index ++)
                    {
                        ...
                        if (print_fff)
                        {
                            ...
                            // Upstream 2.4.2 replaced the bool check_multi_filaments_compatibility(types)
                            // with a temperature-range based variant; keep the fork's high/low temp-type
                            // rule via get_filament_temp_type() + is_filaments_compatible().
                            std::vector<int> ftemp_types;
                            ftemp_types.reserve(extruders.size());
                            for (unsigned int eid : extruders)
                                ftemp_types.push_back(Print::get_filament_temp_type(pcfg.filament_type.get_at(eid)));
                            if (!Print::is_filaments_compatible(ftemp_types)) {
                                boost::nowide::cerr
