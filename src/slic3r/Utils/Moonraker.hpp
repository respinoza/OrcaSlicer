...
//
// Snapmaker: this is also the base of the Snapmaker hosts (host types htMoonRaker and
// htMoonRaker_mqtt / class Moonraker_Mqtt below). Constructed with snapmaker_flavor = true it keeps
// the Snapmaker Orca behaviour (connection test via /printer/info, WebPreprintDialog before a
// start-print upload, "print" form field, port-8080 fallback, Windows mDNS/IP resolve workaround,
// :1884/:8883 MQTT port stripping in make_url(), WCP logging). With the default flavor it is
// upstream OrcaSlicer's generic Moonraker host (htMoonraker).
class Moonraker : public PrintHost
{
public:
    Moonraker(DynamicPrintConfig *config, bool snapmaker_flavor = false);
    ...
    bool has_auto_discovery() const override { return m_snapmaker_flavor; }
    ...
    // Snapmaker: async printer information methods (implemented by Moonraker_Mqtt)
...
protected:
    ...
    // Snapmaker: true for htMoonRaker / Moonraker_Mqtt (fork behaviour), false for upstream's htMoonraker.
    bool        m_snapmaker_flavor { false };

    // Snapmaker: time synchronization manager
    ...
    // Snapmaker: internal upload implementations (snapmaker flavor)
#ifdef WIN32
...
private:
    // Snapmaker flavor of test() / upload()
    bool test_snapmaker(wxString &msg) const;
    bool upload_snapmaker(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const;
#ifdef WIN32
