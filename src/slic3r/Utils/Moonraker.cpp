...
Moonraker::Moonraker(DynamicPrintConfig *config, bool snapmaker_flavor)
    : m_host(config->opt_string("print_host"))
    , m_apikey(config->opt_string("printhost_apikey"))
    , m_cafile(config->opt_string("printhost_cafile"))
    , m_ssl_revoke_best_effort(config->opt_bool("printhost_ssl_ignore_revoke"))
    , m_snapmaker_flavor(snapmaker_flavor)
...
{
    // Snapmaker: keep the fork's wording for the Snapmaker hosts
    if (m_snapmaker_flavor)
...
std::string Moonraker::make_url(const std::string &path) const
{
    ...
    if (m_snapmaker_flavor) {
        // Snapmaker: the print_host of a Snapmaker (MQTT) host may carry the MQTT port; HTTP goes to the default port.
...
{
    if (m_snapmaker_flavor) {
        // Snapmaker: fork behaviour (always send X-Api-Key, WCP logging)
        auto& wcp_loger = GUI::WCP_Logger::getInstance();
...
{
    // Snapmaker: the Snapmaker hosts keep the fork's /printer/info connection test
    if (m_snapmaker_flavor)
        return test_snapmaker(msg);
...
{
    // Snapmaker: the Snapmaker hosts never offered a storage picker (fork behaviour)
    if (m_snapmaker_flavor)
        return PrintHost::get_storage(storage_path, storage_name);
...
{
    // Snapmaker: the Snapmaker hosts keep the fork's upload flow (preprint dialog, "print" form field,
    // port-8080 fallback, Windows IP-resolve workaround)
    if (m_snapmaker_flavor)
        return upload_snapmaker(std::move(upload_data), progress_fn, error_fn, info_fn);
...
// ---------------------------------------------------------------------------------------------
// Snapmaker flavor of the Moonraker host (fork code)
// ---------------------------------------------------------------------------------------------
...
bool Moonraker::test_snapmaker(wxString& msg) const
{
    auto& wcp_loger = GUI::WCP_Logger::getInstance();    
...
bool Moonraker::upload_snapmaker(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    auto& wcp_loger = GUI::WCP_Logger::getInstance();    
...
Moonraker_Mqtt::Moonraker_Mqtt(DynamicPrintConfig* config, bool change_engine) : Moonraker(config, true) {
