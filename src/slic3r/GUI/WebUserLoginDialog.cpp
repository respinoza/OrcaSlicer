    ...
    : wxDialog((wxWindow*) (wxGetApp().mainframe), wxID_ANY, "Snapmaker Orca"), m_cloud_agent(cloud_agent)
{
    ...
    // Snapmaker Orca: branded window icon (fork set it on the plug-in-missing page; applied to every page here)
    ...
    if (m_cloud_agent->get_id() == BBL_CLOUD_PROVIDER && !bblnetwork_enabled) {
        ...
        // Snapmaker Orca: fork's own user agent (member renamed m_bbl_user_agent -> m_sm_user_agent in the header)
