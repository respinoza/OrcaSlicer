...
// MERGE(2.4.2): the fork's static MACHINE_BED_TYPE_STRING[_EX/_ALL] arrays and
// SelectMachineDialog::stripWhiteSpace() were dropped here: upstream replaced the whole
// bed-type-string mechanism with the dynamic MACHINE_BED_TYPE_STRING vector +
// init_machine_bed_types() below (which now also carries "Bambu Cool Plate SuperTack"),
// and stripWhiteSpace's declaration is already gone from SelectMachine.hpp with no
// remaining callers in this file. The fork's "Graphic Effect Plate" entry (from its old
// MACHINE_BED_TYPE_STRING_ALL array) is carried over as the last entry of
// init_machine_bed_types(), after upstream's entries so upstream's indices stay unchanged.
...
void                SelectMachineDialog::init_machine_bed_types()
{
    if (MACHINE_BED_TYPE_STRING.size() == 0) {
        MACHINE_BED_TYPE_STRING = {//_L("Auto"),
                                   ...
                                   // Snapmaker: Graphic Effect Plate (fork's MACHINE_BED_TYPE_STRING_ALL)
                                   _L("Graphic Effect Plate")
        };
        MachineBedTypeString    = {//"auto",
                                   "pc", "pe", "pei", "pte", "suprtack", "gesp"};
    }
}
