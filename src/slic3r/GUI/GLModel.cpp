...
bool GLModel::smooth_normals_from_app_config()
{
    // Reads AppConfig: call on the main thread only.
    const AppConfig* app_config = wxGetApp().app_config;
    return app_config != nullptr && app_config->get_bool(SETTING_OPENGL_REALISTIC_MODE) &&
           app_config->get_bool(SETTING_OPENGL_PHONG_SMOOTH_NORMALS);
}

void GLModel::init_from(const indexed_triangle_set& its)
{
    init_from(its, smooth_normals_from_app_config());
}
...
void GLModel::init_from(const indexed_triangle_set& its, bool smooth_normals)
{
    ...
    // smooth_normals is sampled by the caller (smooth_normals_from_app_config() on the main thread),
    // so no AppConfig access happens here when this runs on a worker thread.
    if (smooth_normals) {
