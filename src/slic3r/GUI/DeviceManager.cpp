...
{
    // MERGE-TODO(2.4.2): upstream moved DeviceManager::insert_local_device() and the other
    // DeviceManager:: helper functions that used to live in this spot into
    // src/slic3r/GUI/DeviceCore/DevManager.cpp. Keeping a second copy here would duplicate
    // symbols, so this file now only carries upstream's new door-open-check body below.
    // The fork's one real customization to the moved insert_local_device() body -
    // "::Slic3r::SnapLog::v1::SnapLogClient::instance().set_device_id(dev_id);" right after
    // dev_id is known - still needs to be re-applied inside DeviceCore/DevManager.cpp.
    json j;
