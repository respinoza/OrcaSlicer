        ...
        if (selected_filament_preset) {
            if (m_custom_range_panel) {
                ...
                if (speed_opt) {
                    ...
                    // MERGE-TODO(2.4.2): qualified as Slic3r:: to disambiguate from the member
                    // CalibrationPresetPage::get_nozzle_volume_type(int) added by upstream 2.4.2,
                    // which otherwise shadows this free function from PresetFlowVariant.hpp via ADL.
                    const FilamentVolumeType volume_type = printer_preset == nullptr ? fvtStandard :
                        Slic3r::get_nozzle_volume_type(printer_preset->config, 0);
