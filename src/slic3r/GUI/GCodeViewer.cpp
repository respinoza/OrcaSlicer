                ...
                const std::vector<BoundingBoxf3>& exclude_bounding_box, ConfigOptionMode mode, bool only_gcode)
{
    ...
    // the previous gcode's data would leak / be mixed into the new load.
    reset();

    // A GUI event dispatched while the load below is in progress can re-enter
    // reset() (via reset_gcode_toolpaths) and wipe the viewer state mid-flight.
    // Arm the guard so any re-entrant reset() is ignored. ScopeGuard clears the
    // flag on every exit, including exceptions thrown during the load.
    m_loading = true;
    ...
    gcode_result.lock();
    // Unlock the result and re-enable background processing on every exit path
    // (early returns and exceptions included).
    ScopeGuard unlock_guard([&gcode_result]() {
        gcode_result.unlock();
...
// #endif // !VGCODE_ENABLE_COG_AND_TOOL_MARKERS
    ...
    // run here via the ScopeGuards above (also on every early return / exception).
    //BBS: add logs
...
void GCodeViewer::render_all_plates_stats(const std::vector<const GCodeProcessorResult*>& gcode_result_list, bool show /*= true*/) const {
    ...
    // title and item data
    {
        ...
        for (auto plate : plate_list.get_nonempty_plate_list())
        {
            ...
            // from BambuStudio (STUDIO-15763). Applied to every extruder present in the plate
            // statistics (not only plate->get_extruders()), so flush/tower/support-only
            // filaments are still listed.
            const size_t max_extruders_colors = wxGetApp().plater()->get_extruders_colors().size();
            ...
            for (const auto& [extruder_id, flush_volume] : plate_print_statistics.flush_per_filament) {
                if (extruder_id >= max_extruders_colors)
                    continue;
                flushed_volume_of_extruders_all_plates[static_cast<int>(extruder_id)] += flush_volume;
            }
...
// ORCA: Add Pressure Advance visualization support
    ...
    if (new_view_type == libvgcode::EViewType::ColorPrint) {
        ...
        auto generate_partial_times = [this, get_used_filament_from_volume](const TimesList& times, const std::vector<double>& used_filaments) {
            ...
            const libvgcode::Palette& viewer_tool_colors = m_viewer.get_tool_colors();
            for (size_t i = 0; i < last_color_count && i < viewer_tool_colors.size(); ++i) {
                last_color[i] = libvgcode::convert(viewer_tool_colors[i]);
            }
