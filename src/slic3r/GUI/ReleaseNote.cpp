    ...
    : DPIDialog(parent, wxID_ANY, _L("New version of Snapmaker Orca"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER)
{
    ...
    github_link->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e) {
        if (isModal)
            ...
            // Snapmaker Orca shows this dialog modeless (setDialogMode(false)); EndModal would assert there,
            // so open the download page the same way the Download button does.
    ...
    // Snapmaker Orca keeps a single UpdateVersionDialog alive and reuses it for every update check
    // (GUI_App::m_updateDialog), so upstream's per-instance navigation counter would go stale after the
    // first showing. Decide by URL instead: update_version_info() loads the guide page, which is swapped
    // for the rendered release note; http(s) links clicked by the user open in the default browser.
    m_vebview_release_note->Bind(wxEVT_WEBVIEW_NAVIGATING,[=](wxWebViewEvent& event) {
        wxString nav_url = event.GetURL();
        nav_url.Replace("\\", "/");
        if (nav_url.EndsWith("web/guide/0/index.html")) {
            m_vebview_release_note->SetPage(wxString::FromUTF8(html_source), "");
        } else if (nav_url.StartsWith("http://") || nav_url.StartsWith("https://")) {
	...
	// Release notes are rendered from markdown in update_version_info(); the static page is no longer loaded.
	// fs::path ph(data_dir());
