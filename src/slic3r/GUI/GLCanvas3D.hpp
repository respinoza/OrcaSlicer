...
public:
    ...
    void render_thumbnail(ThumbnailData &           thumbnail_data,
                                 ...
                                 bool                      for_picking  = false,
                                 bool                      ban_light    = false);
    // Snapmaker: named-viewpoint overload used by MixedFilamentBatchDialog (see ThumbnailView.hpp).
    // Upstream 2.4.x replaced the bool use_top_view parameter with Camera::ViewAngleType; this
    // overload maps the fork's ThumbnailView onto Camera::ViewAngleType (GLCanvas3D.cpp) and
    // renders through the upstream path with explicit volumes and extruder colours.
    void render_thumbnail(ThumbnailData& thumbnail_data, unsigned int w, unsigned int h, const ThumbnailsParams& thumbnail_params,
                                 const GLVolumeCollection &volumes,
                                 std::vector<ColorRGBA>&   extruder_colors,
                                 Camera::EType             camera_type,
...
private:
    ...
    // (render_thumbnail_legacy is now the public static ViewAngleType overload above.)
