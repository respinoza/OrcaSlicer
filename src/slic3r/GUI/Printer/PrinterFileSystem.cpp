...
struct PrinterFileSystem::Upload : Progress
{
    ...
    // Snapmaker Orca: boost md5 (as Download uses) instead of OpenSSL's deprecated MD5_CTX API;
    // the fork's Utils.hpp only pulls in <openssl/evp.h>.
    boost::uuids::detail::md5   boost_md5;
...
int PrinterFileSystem::UploadFileTask(std::shared_ptr<UploadFile> upload_file, boost::uint64_t seq, std::string &msg)
{
    ...
    if (!upload->ifs.is_open()) {
        ...
        upload->boost_md5 = boost::uuids::detail::md5();
    }
    ...
    upload->boost_md5.process_bytes(buffer, read_size);
    upload->size += read_size;
    if (upload->size == upload->total) {
        boost::uuids::detail::md5::digest_type digest;
        upload->boost_md5.get_digest(digest);
        for (int i = 0; i < 4; ++i) digest[i] = boost::endian::endian_reverse(digest[i]);
        std::string md5_out;
        const auto  char_digest = reinterpret_cast<const char *>(&digest[0]);
        boost::algorithm::hex(char_digest, char_digest + sizeof(digest), std::back_inserter(md5_out));
