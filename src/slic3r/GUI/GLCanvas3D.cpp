...
// Snapmaker: named-viewpoint overload used by MixedFilamentBatchDialog (ThumbnailView.hpp).
// Upstream 2.4.x added Camera::ViewAngleType with the same set of views, so the fork's
// ThumbnailView is mapped onto it and rendered through the upstream path. Unlike upstream's
// explicit-colours overload it keeps the "flat" shader for picking renders.
static Camera::ViewAngleType thumbnail_view_to_view_angle_type(ThumbnailView view)
{
    switch (view) {
    ...
    case ThumbnailView::Iso:
    default:                      return Camera::ViewAngleType::Iso;
    }
}
                                  ...
                                  bool                      ban_light)
{
    ...
    if (for_picking)
        ...
        shader = wxGetApp().get_shader("thumbnail");
    ModelObjectPtrs& model_objects = GUI::wxGetApp().model().objects;
    const Camera::ViewAngleType camera_view_angle_type = thumbnail_view_to_view_angle_type(view);
    switch (OpenGLManager::get_framebuffers_type())
    {
    case OpenGLManager::EFramebufferType::Arb:
                                           ...
                                           bool                               ban_light)
{
    ...
    if (camera_view_angle_type == Camera::ViewAngleType::Top_Plate) {
        ...
        // Snapmaker: re-lock the caller's projection for named views (select_view("topfront")
        // could otherwise flip an Ortho thumbnail camera to Perspective via auto_type).
        if (camera_view_angle_type != Camera::ViewAngleType::Iso)
            camera.set_type(camera_type);
    }
...
void GLCanvas3D::_render_objects(GLVolumeCollection::ERenderType type, bool with_outline)
{
    ...
    if (shader != nullptr) {
        ...
        // Snapmaker: GLVolumeCollection::render takes the Camera directly, so the canvas size
        // upstream passed alongside view/projection matrices is no longer needed here.
        {
            const Camera& camera = wxGetApp().plater()->get_camera();
            shader->set_uniform("z_far", camera.get_far_z());
            shader->set_uniform("z_near", camera.get_near_z());
    ...
    for (int i = 0; i < plate_list.get_plate_count(); i++) {
        if (i < m_sel_plate_toolbar.m_items.size()) {
            ...
            if (plate_list.get_plate(i)->is_slice_result_valid()) {
                ...
                if (plate_list.get_plate(i)->has_printable_instances() && !wxGetApp().plater()->is_plate_sliceable(i))
                    m_sel_plate_toolbar.m_items[i]->slice_state = IMToolbarItem::SliceState::SLICE_FAILED;
                else {
...
void GLCanvas3D::_render_canvas_toolbar() 
{
    ...
    if (ImGui::BeginPopup("CanvasToolbarMenu")) {
        ...
        // Snapmaker: upstream's "Outline" toggle is hidden. It drives GLVolume::render_with_outline(), which the
        // fork removed on purpose (87f34b9a6); selection is highlighted by brighten_color() instead.
