...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, const bool is_global_config, size_t flow_variant_index)
{
    ...
    if (junction_deviation_enabled) {
        ...
        // default_jerk is a per-flow-variant vector (ConfigOptionFloats) in this fork
        const bool have_default_jerk = process_flow_float("default_jerk", 0) > 0;
        for (auto el : jerk_options) {
            toggle_line(el, true);
...
//    toggle_line("support_interface_speed", have_support_material && have_support_interface);
    ...
    // Upstream: Type-1 (BBL) towers only take a user width for rib walls; Type-2 towers always do.
    // Snapmaker fork (square wipe tower, #325): WipeTower2 squares a rib-wall tower itself, so the
    // width is not user-settable there.
    toggle_field("prime_tower_width", have_prime_tower && (supports_wipe_tower_2 ? !have_rib_wall : have_rib_wall));
    ...
    // enable_overhang_speed is a per-flow-variant vector (ConfigOptionBools) in this fork
