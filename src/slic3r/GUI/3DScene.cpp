...
bool GLVolume::SimplifyMesh(const indexed_triangle_set& its, std::shared_ptr<GUI::GLModel> model, std::shared_ptr<std::atomic<bool>> readyFlag, LODLevel lod) const
{
    ...
    // Sample the AppConfig-driven normals mode here, on the calling (main) thread: AppConfig
    // must not be read from the worker below, so it gets the flag by value.
    const bool smoothNormals = GUI::GLModel::smooth_normals_from_app_config();
    ...
    std::thread worker = std::thread(
        [model, readyFlag, maxError, originMesh, smoothNormals](std::unique_ptr<indexed_triangle_set> itsPtr) {
            ...
            if (originMin.x() < simplifiedMesh.stats().min.x() &&
                ...
                if (model && model.use_count() >= 2) {
                    ...
                    model->init_from(simplifiedMesh, smoothNormals);
...
// Snapmaker: upstream's GLVolume::render_with_outline() (selected-volume outline, toggled by
// GUI_App::show_outline()) is intentionally not ported: the fork removed it together with its 3DScene.hpp
// declaration and its call in GLVolumeCollection::render(). Upstream's "Outline" toggle in the GLCanvas3D view
// menu is hidden accordingly; selection is highlighted by brighten_color() only.
...
bool GLVolumeCollection::check_outside_state(const BuildVolume &build_volume, ModelInstanceEPrintVolumeState *out_state, ObjectFilamentResults* object_results) const
{
    ...
    for (GLVolume* volume : this->volumes)
    {
        ...
        if (! volume->is_modifier && (volume->shader_outside_printer_detection_enabled || (! volume->is_wipe_tower && volume->composite_id.volume_id >= 0))) {
            ...
            // MERGE(2.4.2): upstream's extruder-area check reports a volume that is fully on the bed
            // but outside some extruder's reachable area as Limited, so Limited counts as inside here.
            if (plate_build_volume.type() == BuildVolume_Type::Rectangle && volume->composite_id.volume_id >= 0 &&
                (state == BuildVolume::ObjectState::Inside || state == BuildVolume::ObjectState::Limited) && volume->printable) {
