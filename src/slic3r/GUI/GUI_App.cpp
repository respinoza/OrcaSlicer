...
public:
        ...
        )
    {
        ...
        // Snapmaker: branded splash (app icon + "Snapmaker Orca" + version tag on a white
        // background), drawn by OnPaint on top of upstream's paint-based splash.
        // init constant texts and scale fonts
        m_constant_text.init();

        // ORCA scale all fonts
        ...
        m_title_color = wxColour(23, 23, 23);
        m_fg_color    = wxColour(143, 143, 143);

        // Logo icon: 140x140 in a 480x480 design
        wxSize sz        = m_window->GetClientSize();
        int    logo_size = sz.GetWidth() * 140 / 480;
        BitmapCache bmp_cache;
        wxBitmap* logo_bmp = bmp_cache.load_svg("splash_app_icon", logo_size, logo_size);
        if (logo_bmp != nullptr)
            m_logo_bmp = *logo_bmp;
    ...
    void OnPaint(wxPaintEvent& evt)
    {
        ...
        const int height     = c_sz.GetHeight();
        ...
        if (m_logo_bmp.IsOk())
            dc.DrawBitmap(m_logo_bmp, scaleX(170), scaleY(80), true);

        // Brand name: "Snapmaker Orca"
        dc.SetFont(m_constant_text.titleFont);
        wxSize brandExt = dc.GetTextExtent(m_constant_text.title);

        // Version tag: "V" + current app version
        dc.SetFont(m_constant_text.versionFont);
        wxSize versionExt = dc.GetTextExtent(m_constant_text.version);
        ...
        dc.DrawText(m_constant_text.title, startX, brandY);

        dc.SetFont(m_constant_text.versionFont);
        dc.SetTextForeground(m_fg_color);
        dc.DrawText(m_constant_text.version, startX + brandExt.GetWidth() + gap, tagY);

        // Dynamic action text (set by SetText)
        dc.SetFont(m_constant_text.loadingFont);
        dc.SetTextForeground(m_fg_color);
        wxRect rc = wxRect(0, scaleY(384), width, dc.GetTextExtent(m_text_action).GetHeight());
...
//#if BBL_HAS_FIRST_PAGE
    ...
    // Snapmaker: a URL open already in flight loads its own project; do not start a blank one.
    if (!m_url_open_pending)
    ...
    if (this->preset_updater) { // G-Code Viewer does not initialize preset_updater.
        CallAfter([this] {
            ...
            // Snapmaker: Preferences forces "stealth_mode" on, where it only means "no Bambu/Orca Cloud".
            // preset_updater->sync() also fetches the Snapmaker profile bundle and version info from
            // Snapmaker's own servers, which the fork always ran at startup, so it is not gated on stealth
            // mode here (upstream 2.4.x gates it). Upstream's Orca/Bambu Cloud login and user-preset sync
            // below and in handle_web_request()/start_sync_user_preset() stay stealth-gated.
            {
            ...
            // Snapmaker: device-tab web resources (flutter) are synced separately from presets.
    ...
    // Snapmaker: suppressed. Snapmaker Orca never synced user profiles through Bambu Cloud and keeps
    // Bambu/Orca Cloud off (stealth mode is forced on), so the notice does not apply to its users; before
    // 2.4.2 its app-config "version" also held the BBS-style SLIC3R_VERSION, not an Orca version.
    // Kept disabled (not deleted) to ease future upstream merges.
    constexpr bool show_orca_cloud_migration_notice = false;
    if (show_orca_cloud_migration_notice && is_editor() && m_last_config_version && m_last_config_version->valid()
    ...
    // Orca 2.4.2: HMSQuery::check_hms_info() is gone and the filament blacklist is now loaded
    // lazily by DevFilaBlacklist, so neither needs a startup call any more.
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current Snapmaker_Orca Version %1% (OrcaSlicer %2%) build %3%") % Snapmaker_VERSION % SoftFever_VERSION % GIT_COMMIT_HASH;
...
bool GUI_App::on_init_inner()
{
    ...
    // Snapmaker: the "Current Snapmaker_Orca Version" line now comes from init_app_config() (upstream moved it there).
...
#endif
    // label colours and fonts are initialized earlier now (upstream moved init_label_colours/init_fonts up)
        ...
        Bind(EVT_SLIC3R_VERSION_ONLINE, [this](const wxCommandEvent& evt) {
            if (this->plater_ != nullptr) {
                ...
                if (!skip_this_version || evt.GetInt() != 0) {
                    ...
                    // Snapmaker: the fork shows a modeless update dialog that opens version_info.url itself.
                    // Upstream 2.4.2 sends MSIX builds to the Microsoft Store here instead; that store page is
                    // OrcaSlicer's product, so it is deliberately not wired into the Snapmaker dialog.
...
bool GUI_App::on_init_network(bool try_backup)
{
    ...
    profiler.note(std::string("network agent created=") + (m_agent ? "true" : "false"));
...
void GUI_App::check_new_version_sf(bool show_tips, bool by_user)
{
    ...
    Http::get(update_url)
        ...
        try {
            ...
            // Snapmaker: version check goes to the Snapmaker update server (fork logic above).
            // Upstream 2.4.2 GitHub-release selection (best_release/best_pre, check_stable_update_only)
            // does not apply to Snapmaker builds and is intentionally not merged here.
...
// Snapmaker: page loading http server (serves the local web pages on PAGE_HTTP_PORT)
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (!wxLocale::IsAvailable(locale_language_info->Language)) {
        ...
        wxString message = "Switching Snapmaker Orca to language " + requested_language_code + " failed.";
...
void GUI_App::open_preferences(size_t open_on_tab, const std::string& highlight_option)
{
    ...
    // Snapmaker: the current canvas can be null (e.g. while the device tab is active); never
    // dereference it unguarded.
    auto focus_current_canvas = [this]() {
        if (this->plater_ == nullptr)
            return;
...
bool GUI_App::check_url_association(std::wstring url_prefix, std::wstring& reg_bin)
{
    ...
    wxString      reg_value;
    if (key_cu.Exists())
        reg_value = key_cu.QueryDefaultValue();
    else if (key_lm.Exists())
        reg_value = key_lm.QueryDefaultValue();
    else
        return false;
