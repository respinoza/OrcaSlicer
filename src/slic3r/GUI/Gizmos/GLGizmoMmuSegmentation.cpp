...
void GLGizmoMmuSegmentation::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    // Snapmaker: palette slots follow the UI filament order (m_display_filament_ids maps a
    // display slot to the actual 1-based filament id, mixed filaments included), drawn with
    // ORCA's draw_color_button(); simple-gradient mixed filaments get a gradient overlay.
    ...
    for (size_t extruder_idx = 0; extruder_idx < n_extruder_colors; ++extruder_idx) {
        ...
        if (is_gradient) {
            ...
            // Average color for the button's solid fill (hidden by the gradient overlay)
            ...
            extruder_color = ColorRGBA(float(avg_r) / 255.f, float(avg_g) / 255.f, float(avg_b) / 255.f, 1.f);
        }
        ...
        if (is_gradient) {
            // --- Gradient path: draw the gradient on top of the button, inside its border rings ---
            ImDrawList* draw_list = ImGui::GetWindowDrawList();
            ImVec2 btn_min = ImGui::GetItemRectMin();
            ImVec2 btn_max = ImGui::GetItemRectMax();
            const float inset = 3.5f * scale;
            ...
            // Number text centered on button (the button's own label is covered by the overlay)
        ...
        // Tooltip: number keys 1-9 select a filament; higher slots are selected by click / input box
...
void GLGizmoMmuSegmentation::render_filament_remap_ui(float window_width, float max_tooltip_width, float scale)
{
    ...
    // Snapmaker: remap slots follow the UI filament order. m_display_filament_ids maps a display
    // slot to the actual 1-based filament id; m_extruder_remap maps display slot -> display slot.
    auto slot_filament_id = [this](size_t slot) -> unsigned int {
        if (slot >= m_display_filament_ids.size())
            return 0;
        const unsigned int filament_id = m_display_filament_ids[slot];
        return (filament_id == 0 || filament_id > m_extruders_colors.size()) ? 0 : filament_id;
    };
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(7.f * scale, 7.f * scale));
    for (size_t src = 0; src < n_extr && src < m_extruder_remap.size(); ++src) {
        const unsigned int src_filament_id = slot_filament_id(src);
        if (src_filament_id == 0 || m_used_filaments.count(size_t(src_filament_id - 1)) == 0)
            continue;
        const unsigned int dst_filament_id = slot_filament_id(m_extruder_remap[src]);
        if (dst_filament_id == 0)
            continue;
        ...
        bool src_clicked = draw_color_button(
            ...
            m_extruders_colors[src_filament_id - 1],    // color
            m_extruders_colors[dst_filament_id - 1],    // mapped_color (shows bubble if not matches with Color)
        ...
        if (ImGui::BeginPopup(pop_id.c_str())) {
            ...
            for (int dst = 0; dst < (int)n_extr; ++dst) {
                const unsigned int popup_filament_id = slot_filament_id(size_t(dst));
                if (popup_filament_id == 0)
                    continue;
                ...
                bool dst_clicked = draw_color_button(
                    ...
                    m_extruders_colors[popup_filament_id - 1],  // color
                    m_extruders_colors[popup_filament_id - 1],  // mapped_color (non fuctional in here)
                    m_extruder_remap[src] == size_t(dst),       // is_active
                    scale
                );
                if (dst_clicked) {
...
void GLGizmoMmuSegmentation::remap_filament_assignments()
{
    ...
    for (size_t src = 0; src < n_extr; ++src) {
        ...
        // Upstream fix: only remap painted states; never remap state 0 (unpainted
        // triangles), otherwise the remap would "paint" unpainted areas.
    ...
    for (ModelVolume* mv : mo->volumes) {
        ...
        // Snapmaker: m_extruder_remap maps display slot -> display slot (UI filament order), so
        // translate the volume's filament id to its slot and the destination slot back to a
        // filament id through m_display_filament_ids.
        int current_ext_id = mv->extruder_id();
        const unsigned int current_filament_id = (current_ext_id > 0) ? unsigned(current_ext_id) : 1u;
        auto current_slot_it = std::find(m_display_filament_ids.begin(), m_display_filament_ids.end(), current_filament_id);
        const size_t current_slot = size_t(std::distance(m_display_filament_ids.begin(), current_slot_it));

        if (current_slot_it != m_display_filament_ids.end() && current_slot < m_extruder_remap.size() &&
            m_extruder_remap[current_slot] < m_display_filament_ids.size()) {
            const unsigned int dest_filament_id = m_display_filament_ids[m_extruder_remap[current_slot]];
            const size_t dest_idx = dest_filament_id > 0 ? size_t(dest_filament_id - 1) : 0;
            if (dest_filament_id != 0 && dest_filament_id != current_filament_id) {
