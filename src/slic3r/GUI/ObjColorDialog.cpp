...
// Fork: rows are wider than upstream's so each cluster cell has room for its "Keep color" button.
...
// MERGE(2.4.2): upstream uses (int) EnforcerBlockerType::ExtruderMax (16 there), but the fork raised ExtruderMax to 255
// for mixed filaments, which does not fit in a char (it wrapped to -1 and disabled every limit). Both sides mean 16 here.
static const char g_max_color = (char) std::min(16, (int) EnforcerBlockerType::ExtruderMax);
...
{
    // Fork: only the appended filament options that some cluster actually uses are created, and they are
    // renumbered contiguously after the existing filaments. Otherwise an unused appended option would leave a
    // gap (a filament with an undefined colour) and the painted filament ids would not match the new list.
    const int          existing_filament_count = static_cast<int>(m_colours.size());
    std::map<int, int> appended_filament_id_map;

    ComboBox *options_combo = (!m_result_icon_list.empty() && m_result_icon_list[0] != nullptr) ? m_result_icon_list[0]->bitmap_combox : nullptr;
    if (options_combo != nullptr && static_cast<int>(options_combo->GetCount()) > existing_filament_count + 1) {
        ...
        for (int combo_selection : selected_appended_indices) {
            if (combo_selection >= static_cast<int>(options_combo->GetCount())) {
                continue;
            }

            wxGetApp().sidebar().add_custom_filament(wxColour(options_combo->GetItemTooltip(combo_selection)));
    ...
    for (int &mapped_filament_id : m_cluster_map_filaments) {
        ...
        if (it != appended_filament_id_map.end() && it->second != mapped_filament_id) {
            mapped_filament_id = it->second;
            remapped           = true;
        }
    }
    if (remapped) {
        // The model was painted with the pre-renumbering ids in deal_thumbnail(); repaint it with the final ids.
        update_filament_ids();
        if (m_obj_in_out.filament_ids.size() > 0) {
            if (m_obj_in_out.deal_vertex_color) {
                Model::obj_import_vertex_color_deal(m_obj_in_out.filament_ids, m_obj_in_out.first_extruder_id, m_obj_in_out.model);
            } else {
                Model::obj_import_face_color_deal(m_obj_in_out.filament_ids, m_obj_in_out.first_extruder_id, m_obj_in_out.model);
            }
        }
    }
}
...
{
    // Fork: "Color match" maps onto the existing filaments only, so drop any appended filament options first.
    if (!m_result_icon_list.empty() && m_result_icon_list[0]->bitmap_combox != nullptr &&
        m_result_icon_list[0]->bitmap_combox->GetCount() > m_colours.size() + 1) {
        deal_reset_btn();
    }
...
int ObjColorPanel::find_filament_selection_by_color(const wxColour &color) const
{
    ...
    // Appended (not yet created) filament options exist only as items of the map combo boxes, after the
    // existing filaments. Their tooltips hold the colour as #RRGGBB, so compare RGB only.
    ComboBox *options_combo = (!m_result_icon_list.empty() && m_result_icon_list[0] != nullptr) ? m_result_icon_list[0]->bitmap_combox : nullptr;
    if (options_combo != nullptr) {
        for (unsigned int i = m_colours.size() + 1; i < options_combo->GetCount(); ++i) {
            const wxColour option_color(options_combo->GetItemTooltip(i));
            if (option_color.Red() == color.Red() && option_color.Green() == color.Green() && option_color.Blue() == color.Blue()) {
...
int ObjColorPanel::append_new_filament_option(const wxColour &color)
{
    ...
    // item 0 is the "undefined" entry; the rest are existing filaments followed by appended options
    const int option_count = static_cast<int>(options_combo->GetCount()) - 1;
    if (option_count >= g_max_color) {
        return 0;
    }

    const int selection = option_count + 1;
...
void ObjColorPanel::deal_keep_color_btn(int id)
{
    ...
    if (id < static_cast<int>(m_new_add_colors.size())) {
        m_new_add_colors[id] = m_result_icon_list[id]->bitmap_combox->GetItemTooltip(selection);
    }
...
void ObjColorPanel::draw_new_table()
{
    ...
    auto col       = 2; // fork: 2 cells per row (upstream 3) so each cell fits the "Keep color" button
...
{
    // Fork: map clusters onto the existing filaments by default; the user keeps individual cluster colours
    // with the per-row "Keep color" button (or all of them with "Append").
    if (m_colours.empty()) {
        deal_add_btn();
        return;
    }
...
void ObjColorPanel::generate_thumbnail()
{
    if (m_deal_thumbnail_flag && m_obj_in_out.model->objects.size() == 1) {
        ...
        if (colors.size() > m_colours.size()) {
            colors.resize(m_colours.size());
        }
        // Fork: appended filament options can be added one cluster at a time ("Keep color"), so their colours are
        // taken from the map combo boxes, whose items after the existing filaments are exactly the appended options.
        ComboBox *options_combo = (!m_result_icon_list.empty() && m_result_icon_list[0] != nullptr) ? m_result_icon_list[0]->bitmap_combox : nullptr;
        if (options_combo != nullptr) {
            for (unsigned int i = m_colours.size() + 1; i < options_combo->GetCount(); i++) {
                const wxColour    option_color(options_combo->GetItemTooltip(i));
                Slic3r::ColorRGBA temp_color;
                temp_color[0] = option_color.Red() / 255.f;
                temp_color[1] = option_color.Green() / 255.f;
                temp_color[2] = option_color.Blue() / 255.f;
                temp_color[3] = option_color.Alpha() / 255.f;
...
// Returns true when not every cluster colour could be appended (filament limit reached).
bool ObjColorPanel::deal_add_btn()
{
    if (m_colours.size() >= g_max_color) { return true; }
    deal_reset_btn();
    ...
    for (size_t i = 0; i < m_cluster_colours.size() && i < appended_selections.size(); i++) {
        ...
        if (i < m_new_add_colors.size()) {
            m_new_add_colors[i] = m_result_icon_list[i]->bitmap_combox->GetItemTooltip(appended_selections[i]);
        }
    }
...
wxBoxSizer *ObjColorPanel::create_color_icon_map_rgba_sizer(wxWindow *parent, int id, const wxColour &color)
{
    ...
    // Fork: per-cluster "Keep color" button, shown while the cluster colour is not yet a filament option.
    ...
    icon_sizer->Add(keep_color_btn, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 0);
    return icon_sizer;
}
