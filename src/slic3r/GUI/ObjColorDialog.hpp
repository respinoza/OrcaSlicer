...
private:
    ...
    // Snapmaker: the fork's per-row result sizers (create_result_button_sizer / create_color_icon_and_rgba_sizer /
    // update_color_icon_and_rgba_sizer) were superseded by upstream's map layout in ObjColorDialog.cpp.
...
private:
    ...
    //result
    bool                        m_is_add_filament{false}; // upstream; still set by update_new_add_final_colors()
