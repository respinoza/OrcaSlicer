...
wxBoxSizer *PreferencesDialog::create_item_region_combobox(wxString title, wxString tooltip)
{
    ...
    // Snapmaker: the China region is labelled "Chinese Mainland" (display and stored config value)
...
void PreferencesDialog::create_items()
{
    ...
    //// GENERAL > User Experience (Snapmaker)
...

    {
        ...
        if (region.empty() || region != "zh_CN")
            hyperlink = new wxHyperlinkCtrl(m_parent, wxID_ANY, _L("What data would be collected?"), enUrl);
        else
            hyperlink = new wxHyperlinkCtrl(m_parent, wxID_ANY, _L("What data would be collected?"), cnUrl);
    ...
    auto allow_filament_temp_mixing = create_item_checkbox(_L("Allow high/low temperature filament mixing"),
    ...
    auto item_remember_login = create_item_checkbox(_L("Stay signed in"),
        ...
        "remember_login");
    g_sizer->Add(item_remember_login);

    // Since v2.4.0 the running app refreshes its token on its own (upstream #809), so this
    // toggle only claims what the fork adds: restoring an expired session at startup.
    auto item_auto_renew_login = create_item_checkbox(_L("Automatically stay signed in"),
    #if defined(__linux__)
        _L("When Snapmaker Orca starts and your saved session has expired, sign you back in "
           "silently instead of asking you to log in again. This stores the site login cookie on "
           "disk in plain text (in the app's own data directory) so the session can be renewed. "
           "Off by default; only takes effect while \"Stay signed in\" is on."),
    #else
        ...
        _L("When Snapmaker Orca starts and your saved session has expired, sign you back in "
           "silently instead of asking you to log in again. The renewal reuses the sign-in cookie "
           "held by the system web view, which keeps its own browser profile outside Snapmaker "
           "Orca's control. Off by default; only takes effect while \"Stay signed in\" is on."),
    #endif
        "auto_renew_login");
    g_sizer->Add(item_auto_renew_login);

    // SM Beta: the stealth mode and network plug-in options stay hidden; stealth mode is forced on
    // and the (Bambu) network plug-in off. In Snapmaker Orca stealth mode only means "no Bambu/Orca
    // Cloud": upstream 2.4.x also gates its Orca Cloud login/info commands, user preset sync and the
    // startup profile sync on it. GUI_App::post_init therefore runs the startup preset/profile sync
    // (Snapmaker bundle OTA) regardless of stealth mode, and the Snapmaker account/device flows
    // (sm_*, SSWCP, silent re-auth) never check it.
    ...
    // Snapmaker: upstream's network test (Bambu/Orca Cloud endpoints) stays hidden, like the network plug-in section.
    ...
    // Snapmaker: hidden; Bambu Cloud login is unavailable while stealth mode is forced on (m_bambu_cloud_checkbox stays null).
    ...
    // Snapmaker: upstream's (Bambu AMS) filament sync mode stays hidden; the default (filament & color) applies.
    ...
    // Snapmaker: hidden; this only affects upstream's Orca Cloud token (the Snapmaker account token is
    // governed by "Stay signed in" above).
    ...
    // SM Beta: temporarily close the network plugin (forced off above)
    ...
    auto item_associate_3mf    = create_item_checkbox(_L("Associate 3MF files to Snapmaker Orca"), _L("If enabled, sets Snapmaker Orca as default application to open 3MF files.") , "associate_3mf");
    ...
    auto item_associate_stl    = create_item_checkbox(_L("Associate STL files to Snapmaker Orca"), _L("If enabled, sets Snapmaker Orca as default application to open STL files.") , "associate_stl");
    g_sizer->Add(item_associate_stl);

    auto item_associate_step   = create_item_checkbox(_L("Associate STEP files to Snapmaker Orca"), _L("If enabled, sets Snapmaker Orca as default application to open STEP files."), "associate_step");
