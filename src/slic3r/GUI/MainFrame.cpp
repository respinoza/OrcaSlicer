    ...
    m_slice_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent& event)
        {
            ...
            // Upstream: BBL dual-nozzle filament-map popup; returns true without popping up for
            // non-BBL printers (so it never stacks on the Snapmaker grouping dialog above).
            bool slice = true;
            ...
            // Same events as start_slice() (exit_gizmo()/update() already done above). The fork does not
            // switch to the Preview tab here (removed upstream-side in the fork's top-cover change).
            if (slice) {
...
static wxMenu* generate_help_menu()
{
    ...
    // Check New Version
    append_menu_item(
        helpMenu, wxID_ANY, _L("Check for Updates"), _L("Check for Updates"),
        [](wxCommandEvent&) {
    ...
    // "Open Network Test" moved up with upstream (next to Troubleshoot Center), keeping the fork's delayed release.
