    ...
    {{L("Quality"),
     ...
     {L("Strength"),
      {{"wall_loops", "", 1},
       ...
       {"sparse_infill_filament_id", "", 1}, // Snapmaker fork: per-part infill filament (was "sparse_infill_filament")
    ...
    for (const auto& model : handy_models) {
        append_menu_item(
            ...
            [&model](wxCommandEvent&) {
                ...
                Model& plater_model = currentPlater->model();
                ...
                {
                    if (objectIndex < plater_model.objects.size())
                    {
                        loadedObjects.push_back(plater_model.objects[objectIndex]);
                    }
                }
                plater_model.InitializeAssemblyPositions(loadedObjects);

                if (model.arrange_after_import) {
                    currentPlater->set_prepare_state(Job::PREPARE_STATE_MENU);
                    currentPlater->arrange();
                }
...
void MenuFactory::create_filament_action_menu(bool init, int active_filament_menu_id)
{
    ...
    // Snapmaker fork: the Merge submenu is not gated on Sidebar::should_show_SEMM_buttons(); the U1 toolchanger is neither
    // SEMM nor a BBL printer and must keep Delete / Merge for physical and mixed (Full Spectrum) filaments. The fork's
    // Sidebar::should_show_SEMM_buttons() returns true for every printer, so upstream's Delete gating below is kept as-is.
    auto append_delete_item = [this, menu]() {
