...
void ObjectList::create_objects_ctrl()
{
    ...
    // Snapmaker: always offer the "Default" filament entry (fork fix 2.3.5-30); upstream's
    // context-item check is kept below for reference.
...
void ObjectList::update_filament_values_for_items(const size_t filaments_count)
{
    for (size_t i = 0; i < m_objects->size(); ++i)
    {
        ...
        // Snapmaker Orca: also clamp the per-feature filament overrides (fork's wall/infill filament keys,
        // renamed upstream to the *_filament_id family).
...
void ObjectList::update_filament_values_for_items_when_delete_filament(const size_t filament_id, const int replace_id)
{
    ...
    for (size_t i = 0; i < m_objects->size(); ++i) {
        ...
        // Snapmaker Orca: also remap the per-feature filament overrides (fork's wall/infill filament keys,
        // renamed upstream to the *_filament_id family).
        static const char* keys[] = {"outer_wall_filament_id", "inner_wall_filament_id", "sparse_infill_filament_id",
                                     "internal_solid_filament_id", "top_surface_filament_id", "bottom_surface_filament_id",
                                     "support_filament", "support_interface_filament"};
